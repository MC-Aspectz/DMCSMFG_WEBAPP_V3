...
<head>
    ...
    <?php guideInclude(); ?>
    <!-- -------------------------------------------------------------------------------- -->
    <title><?=$_SESSION['APPNAME'].' - '.$data['DRPLANG']['APPCODE']['SEARCHSALETRAN_ACC2']; ?></title>
</head>
<body>
<main>
    ...
    <form class="w-full h-screen p-2" method="POST" id="saletrans" name="saletrans" onkeydown="if(event.keyCode == 13) { event.preventDefault(); return false;}">
        <div class="flex mb-1">
            <div class="flex w-5/12">
                <label class="text-color block text-sm w-3/12 pr-2 pt-1"><?=checklang('DNCN_NO'); ?></label>
        ...
        <div class="flex mb-1">
            <div class="flex w-5/12">
                ...
                <select id="P5" name="P5" class="text-control shadow-md border px-3 h-7 w-5/12 text-left text-sm rounded-xl border-gray-300 focus:ring-blue-500 focus:border-blue-500">
                    <option value="" <?=(isset($P5) && $P5 === '') ? 'selected' : $data['DRPLANG']['ACCSTSTAUTS']['B'] ?>></option>
            ...
            <table id="table_result" class="w-full border-collapse border border-slate-500">
                <thead class="w-full bg-gray-100">
                    <tr class="flex w-full divide-x csv-row">
                        <th class="w-2/12 text-center csv-col">
                            <span class="text-color text-sm font-semibold tracking-wide whitespace-nowrap"><?=checklang('DNCN_NO'); ?></span>
                        </th>
                        <th class="w-2/12 text-center csv-col">
                            <span class="text-color text-sm font-semibold tracking-wide whitespace-nowrap"><?=checklang('DC_NO'); ?></span>
                        </th>
                        <th class="w-1/12 text-center csv-col">
                            <span class="text-color text-sm font-semibold tracking-wide whitespace-nowrap"><?=checklang('LINE'); ?></span>
                        </th>
                         <th class="w-2/12 text-center csv-col">
                            <span class="text-color text-sm font-semibold tracking-wide whitespace-nowrap"><?=checklang('ITEMCD'); ?></span>
                        </th>
                         <th class="w-3/12 text-center csv-col">
                ...
                foreach ($tdata as $item) {  ?>
                    <tr class="flex w-full p-0 divide-x csv-row">
                        ...
                        <td class="h-6 w-2/12 pl-1 text-sm csv-col"><?=$item['SALETRANNO'] ?></td>
                        <td class="h-6 w-2/12 pl-1 text-sm csv-col"><?=$item['SALEORDERNO'] ?></td>
                        <td class="h-6 w-1/12 pl-1 text-sm csv-col"><?=$item['SALETRANLN'] ?></td>
                        <td class="h-6 w-2/12 pl-1 text-sm csv-col"><?=$item['ITEMCD'] ?></td>
                        <td class="h-6 w-3/12 pl-1 text-sm csv-col"><?=$item['ITEMNAME'] ?></td>
                        <td class="h-6 w-2/12 pl-1 text-sm csv-col"><?php foreach ($accststatus as $i => $v) { echo $item['STATUS'] == $i ? $v : ''; } ?></td>
                    </tr><?php }
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
               <table class="w-full border-collapse border border-slate-500" id="tabel_modal" rules="cols" cellpadding="3" cellspacing="1" >
                    ...
                    <tbody>
                            <td class="text-left pl-1 border border-slate-700 text-sm"><?=checklang('DNCN_NO'); ?></td>
                            <td class="text-left pl-1 border border-slate-700 text-sm" id="DNCN_NO"></td>
                        </tr>
                        <tr>
                            <td class="text-left pl-1 border border-slate-700 text-sm" style="background-color:#ffe6cc;"><?=checklang('DC_NO'); ?></td>
                            <td class="text-left pl-1 border border-slate-700 text-sm" id="DC_NO" style="background-color:#ffe6cc;"></td>
                        </tr>
                        <tr>
                        ...
                        <tr>
                            <td class="text-left pl-1 border border-slate-700 text-sm" style="background-color:#ffe6cc;"><?=checklang('ITEMCD'); ?></td>
                            <td class="text-left pl-1 border border-slate-700 text-sm" id="ITEMCD" style="background-color:#ffe6cc;"></td>
                        </tr>
                        <tr>
                        ...
                        <tr>
                            <td class="text-left pl-1 border border-slate-700 text-sm" style="background-color:#ffe6cc;"><?=checklang('STATUS'); ?></td>
                            <td class="text-left pl-1 border border-slate-700 text-sm" id="STATUS" style="background-color:#ffe6cc;"></td>
                        </tr>
                    </tbody>
                </table>
                <br>
                <div class="h-6 text-[12px]"><?=checklang('ROWCOUNT'); ?> 6</div>
            </div>
