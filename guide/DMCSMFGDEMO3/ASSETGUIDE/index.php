...
<head>
    ...
    <?php guideInclude(); ?>
    <!-- -------------------------------------------------------------------------------- -->
    <title><?=$_SESSION['APPNAME'].' - '.$data['DRPLANG']['APPCODE']['ASSETGUIDE']; ?></title>
</head>
<body>
<main>
    ...
    <form class="w-full h-screen p-2" method="POST" id="assetguide" name="assetguide" onkeydown="if(event.keyCode == 13) { event.preventDefault(); return false;}">       
        <div class="flex mb-1">
            <div class="flex w-6/12">
                ...
                <input type="text" class="text-control text-sm shadow-md border z-20 rounded-xl h-7 w-3/12 py-2 px-3 text-gray-700 border-gray-300"
                    name="VONO" id="VONO" value="<?=isset($data['VONO']) ? $data['VONO']: ''; ?>"/>                  
            </div>
            ...
            <table class="w-full border-collapse border border-slate-500" id="table_result">
                <thead class="w-full bg-gray-100">
                    <tr class="flex w-full divide-x">
                        <th class="w-2/12 text-center pl-1"><?=checklang('VOUCHER_NO'); ?></th>
                        <th class="w-2/12 text-center pl-1"><?=checklang('ASSETCODE'); ?></th>
                        <th class="w-3/12 text-center pl-1"><?=checklang('ASSETNM'); ?></th>
                        <th class="w-3/12 text-center pl-1"><?=checklang('ASSETNM_E'); ?></th>
                        <th class="w-2/12 text-center pl-1"><?=checklang('PURCHASEPLACE'); ?></th>
                    </tr>
                </thead>
                ...
                if (!empty($tdata)) { $minrow = count($tdata) + 1;
                    foreach ($tdata as $key => $item) { ?>
                        <tr class="flex w-full p-0 divide-x">
                            ...
                            <td class="h-6 w-2/12 text-sm text-left pl-1"><?=isset($item['VONO']) ? $item['VONO']: '' ?></td>
                            <td class="h-6 w-2/12 text-sm text-left pl-1"><?=isset($item['ASSETCODE']) ? $item['ASSETCODE']: '' ?></td>
                            <td class="h-6 w-3/12 text-sm text-left pl-1"><?=isset($item['ASSETNM']) ? $item['ASSETNM']: ''?></td>
                            <td class="h-6 w-3/12 text-sm text-left pl-1"><?=isset($item['ASSETNM_E']) ? $item['ASSETNM_E']: '' ?></td>
                            <td class="h-6 w-2/12 text-sm text-left pl-1"><?=isset($item['PURCHASEPLACE']) ? $item['PURCHASEPLACE']: '' ?></td>
                        </tr> <?php 
                    }  
                } 
                ...
                for ($i = $minrow; $i <= $maxrow; $i++) {  ?>
                    <tr class="flex w-full p-0 divide-x">
                        <td class="h-6 w-2/12 text-sm text-left pl-1"></td>
                        <td class="h-6 w-2/12 text-sm text-left pl-1"></td>
                        <td class="h-6 w-3/12 text-sm text-left pl-1"></td>
                        <td class="h-6 w-3/12 text-sm text-left pl-1"></td>
                        <td class="h-6 w-2/12 text-sm text-left pl-1"></td>
                    </tr><?php
                } ?> 
