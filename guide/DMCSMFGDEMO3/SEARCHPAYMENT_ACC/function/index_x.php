...
if(isset($_POST['search'])) {
	...
	// print_r($Param);
  	$tdata = $javaFunc->searchPayment($Param);
	// echo '<pre>';
	// print_r($tdata);
	// echo '</pre>';
}
...
// ------------------------- CALL Langauge -------------------//
$loadApp = getSystemData('SEARCHPAYMENT_ACC');
if(empty($loadApp)) {
    $loadApp = $syslogic->getLoadApp('SEARCHPAYMENT_ACC');
    $syslogic->ProgramRundelete('SEARCHPAYMENT_ACC');
    setSystemData('SEARCHPAYMENT_ACC', $loadApp);
}
// else {
//     $setLoadApp = $syslogic->setLoadApp('SEARCHPAYMENT_ACC');
// }
