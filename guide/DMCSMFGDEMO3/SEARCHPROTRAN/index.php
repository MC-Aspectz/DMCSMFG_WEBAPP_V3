...
<head>
    ...
    <?php guideInclude(); ?>
    <!-- -------------------------------------------------------------------------------- -->
    <title><?php echo $_SESSION['APPNAME'].' - '.$lang['warehousingentry']; ?></title>
</head>
<body>
<main>
    ...
    <form class="w-full h-screen p-2" method="POST" id="searchProtran" name="searchProtran" onkeydown="if(event.keyCode == 13) { event.preventDefault(); return false;}">
        <div class="flex mb-1">
            <div class="flex w-6/12 px-2">
                ...
                <input type="text" class="text-control shadow-md border rounded-xl h-7 w-4/12 py-2 px-3 text-gray-700 border-gray-300 focus:ring-blue-500 focus:border-blue-500 read"
                        id="P1" name="P1" value="<?=$P1 ?>" readonly/>
            ...
            <table id="table_result" class="w-full border-collapse border border-slate-500">
                <thead class="w-full bg-gray-100">
                    <tr class="flex w-full divide-x">
                        ...
                        <th class="w-4/12 text-center">
                            <span class="text-color text-sm font-semibold tracking-wide whitespace-nowrap"><?=$lang['incomingdate']; ?></span>
                        </th>
                        <th class="w-4/12 text-center">
                            <span class="text-color text-sm font-semibold tracking-wide whitespace-nowrap"><?=$lang['receivingqty']; ?></span>
                        </th>
                    </tr>
                </thead>
                ...
                if (!empty($tdata)) { $run = 0; $minrow = count($tdata);
                    foreach ($tdata as $item) {  ?>
                        <tr class="flex w-full p-0 divide-x">
                            ...
                            <td class="h-6 w-4/12 pl-2 text-sm"><?=isset($item['PROTRANORDERNO']) ? $item['PROTRANORDERNO']: '' ?></td>
                            <td class="h-6 w-4/12 pl-2 text-sm"><?=isset($item['PROTRANDT']) ? date('Y/m/d', strtotime($item['PROTRANDT'])): '' ?></td>
                            <td class="h-6 w-4/12 pl-2 text-sm"><?=isset($item['PROTRANQTY']) ? number_format(str_replace(',', '',$item['PROTRANQTY']), 2): '' ?></td>
                        </tr><?php 
            ...
            <div class="modal-body">
               <table class="w-full border-collapse border border-slate-500" id="tb-modal" rules="cols" cellpadding="3" cellspacing="1" >
                    ...
                    <tbody>
                        ...
                        <tr>
                            <td class="text-left pl-1 border border-slate-700 text-sm"><?=$lang['incomingdate'] ?></td>
                            <td class="text-left pl-1 border border-slate-700 text-sm" id="INCOMINGDATE"></td>
                        </tr>
                        <tr>
                            <td class="text-left pl-1 border border-slate-700 text-sm"><?=$lang['receivingqty'] ?></td>
                            <td class="text-left pl-1 border border-slate-700 text-sm" id="RECEIVINGQTY"></td>
                        </tr>
                    </tbody>
                </table>
