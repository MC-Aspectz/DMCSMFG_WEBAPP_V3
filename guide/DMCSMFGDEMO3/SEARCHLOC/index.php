...
<head>
    ...
    <?php guideInclude(); ?>
    <!-- -------------------------------------------------------------------------------- -->
    <title><?=$_SESSION['APPNAME'].' - '.lang('locationindex'); ?></title>
</head>
<body>
    <main>
        ...
        <form class="w-full h-screen p-2" method="POST" id="locationindex" name="locationindex" onkeydown="if(event.keyCode == 13) { event.preventDefault(); return false;}">
            <div class="flex mb-1">
                <div class="flex w-7/12">
                    <label class="text-color block text-sm w-3/12 pr-2 pt-1"><?=lang('loctype'); ?></label>
                    <select class="text-control shadow-md border mr-1 px-3 h-7 w-4/12 text-left text-[12px] rounded-xl border-gray-300 focus:ring-blue-500 focus:border-blue-500 read"
                            id="P1" name="P1" readonly>
                            <option value="" <?=(isset($P1) && $P1 === '') ? 'selected' : '' ?>></option>
                            <?php foreach ($typeItem as $key => $item) { ?>
                                <option value="<?=$key ?>"<?=(isset($P1) && $P1 == $key) ? 'selected' : '' ?>><?=$item?></option>
                            <?php } ?>
                    </select>
                </div>
            ...
            <div class="flex mb-1">
                <div class="flex w-7/12">
                    <label class="text-color block text-sm w-3/12 pr-2 pt-1"><?=lang('loccode'); ?></label>
                    <input class="text-control shadow-md border rounded-xl h-7 w-4/12 py-2 px-3 text-gray-700 border-gray-300 focus:ring-blue-500 focus:border-blue-500"
                           type="text" id="P2" name="P2" value="<?=$P2 ?>"/>
                </div>
                    ...
                    <thead class="sticky top-0 z-20 bg-gray-50">
                        <tr class="border border-gray-600">
                            <th class="px-6 text-center border border-slate-700 text-left">
                                <span class="text-color text-sm font-semibold tracking-wide whitespace-nowrap"><?=lang('loccode'); ?></span>
                            </th>
                            <th class="px-6 text-center border border-slate-700 text-center">
                                <span class="text-color text-sm font-semibold tracking-wide whitespace-nowrap"><?=lang('locname'); ?></span>
                            </th>
                        </tr>
                    </thead>
                        ...
                        if (!empty($tdata)) { $minrow = count($tdata);
                            foreach($tdata as $key => $item) { ?>
                                <tr class="row-id">
                                    ...
                                    <td class="h-6 pl-1 text-sm border border-slate-700 text-left whitespace-nowrap"><?=$item['LOCCD'] ?></td>
                                    <td class="h-6 pl-1 text-sm border border-slate-700 text-left whitespace-nowrap"><?=$item['LOCNAME'] ?></td>
                                </tr><?php 
                            }
                        } 
            ...
            <div class="flex my-2">
                <div class="flex w-7/12">
                    <button type="button" class="btn text-color border-2 focus:ring-4 focus:outline-none focus:ring-gray-300 font-medium rounded-3xl text-sm px-5 py-1 text-center me-2 mb-2" 
                            id="select_item" name="search"><?=lang('select'); ?>
                    </button>
                    <button class="btn text-color border-2 focus:ring-4 focus:outline-none focus:ring-gray-300 font-medium rounded-3xl text-sm px-5 py-1 text-center me-2 mb-2"
                            type="button" id="view_item"><?=lang('view'); ?>
                    </button>
                </div>
                
                <div class="flex w-5/12 justify-end">
                    <button class="btn text-color border-2 focus:ring-4 focus:outline-none focus:ring-gray-300 font-medium rounded-3xl text-sm px-5 py-1 text-center me-2 mb-2" 
                            type="reset" onclick="return clearForm(this.form);" ><?=lang('clear'); ?>
                    </button>
                    <button class="btn text-color border-2 focus:ring-4 focus:outline-none focus:ring-gray-300 font-medium rounded-3xl text-sm px-5 py-1 text-center me-2 mb-2" 
                            type="button" id="back" ><?=lang('back'); ?>
                    </button>
                </div>
            </div>
        </form>
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                ...
                <table class="w-full border-collapse border border-slate-500" id="tabel_modal" rules="cols" cellpadding="3" cellspacing="1" >
                        ...
                        <tbody>
                            <tr>
                                <td class="text-left pl-1 border border-slate-700 text-sm"><?=lang('loccode'); ?></td>
                                <td class="text-left pl-1 border border-slate-700 text-sm" id="loccd"></td>
                            </tr>
                            <tr>
                                <td class="text-left pl-1 border border-slate-700 text-sm" style="background-color:#ffe6cc;"><?=lang('locname'); ?></td>
                                <td class="text-left pl-1 border border-slate-700 text-sm" id="locnm" style="background-color:#ffe6cc;"></td>
                            </tr>
                        </tbody>
                    </table>
                    <br>
