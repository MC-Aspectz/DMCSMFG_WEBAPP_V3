...
<head>
    ...
    <title><?=$_SESSION['APPNAME'].' - '.$lang['partnerIndex']; ?></title>
</head>
<body>
<main>
    ...
    <form class="w-full h-screen p-2" method="POST" id="marketindex" name="marketindex" onkeydown="if(event.keyCode == 13) { event.preventDefault(); return false;}">
        <div class="flex mb-1">
            <div class="flex w-5/12">
                <label class="text-color block text-sm w-3/12 pr-2 pt-1"><?=checklang('PARTNER_TYPE'); ?></label>
                <select class="text-control shadow-md border mr-1 px-3 h-7 w-4/12 text-left text-[12px] rounded-xl border-gray-300 focus:ring-blue-500 focus:border-blue-500 read"
                        ...
                        <option value=""></option>
                        <?php foreach ($PARTNERTYPE as $key => $item) { ?>
                            <option value="<?=$key ?>"<?=(isset($P1) && $P1 == $key) ? 'selected' : '' ?>><?=$item?></option>
                        <?php } ?>
                </select>
                <label class="text-color block text-sm w-5/12 pt-1 text-center"><?=checklang('PARTNER_CODE'); ?></label>
            </div>
        ...
        <div class="table">
            <table id="table_result" class="w-full border-collapse border border-slate-500">
                <thead class="w-full bg-gray-100">
                    <tr class="flex w-full divide-x">
                        <th class="w-4/12 text-center">
                            <span class="text-color text-sm font-semibold tracking-wide whitespace-nowrap"><?=checklang('PARTNER_CODE'); ?></span>
                        </th>
                        <th class="w-6/12 text-center">
                            <span class="text-color text-sm font-semibold tracking-wide whitespace-nowrap"><?=checklang('PARTNER_NAME'); ?></span>
                        </th>
                ...
                <tbody class="flex flex-col overflow-y-scroll w-full h-[350px]"><?php
                if (!empty($tdata)) { $run = 0; $minrow = count($tdata);
                    foreach ($tdata as $item) {  ?>
                        <tr class="flex w-full p-0 divide-x">
                            <td class="hidden"><?=++$run ?></td>
                            <td class="h-6 w-4/12 pl-2 text-sm"><?=isset($item['PARTNERCD']) ? $item['PARTNERCD']: '' ?></td>
                            <td class="h-6 w-6/12 pl-2 text-sm"><?=isset($item['PARTNERNAME']) ? $item['PARTNERNAME']: '' ?></td>
                            <td class="h-6 w-2/12 pl-2 text-sm"></td>
                        </tr><?php 
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                <table class="w-full border-collapse border border-slate-500" id="tb-modal" rules="cols" cellpadding="3" cellspacing="1" >
                    ...
                    <tbody>
                        <tr>
                            <td class="text-left pl-1 border border-slate-700 text-sm"><?=checklang('PARTNERCD') ?></td>
                            <td class="text-left pl-1 border border-slate-700 text-sm" id="PARTNERCD"></td>
                        </tr>
                        <tr>
                            <td class="text-left pl-1 border border-slate-700 text-sm"><?=checklang('PARTNERNAME') ?></td>
                            <td class="text-left pl-1 border border-slate-700 text-sm" id="PARTNERNAME"></td>
                        </tr>
                        <tr>
