...
<head>
    ...
    <?php guideInclude(); ?>
    <!-- -------------------------------------------------------------------------------- -->
    <title><?=$_SESSION['APPNAME'].' - '.$lang['SEARCHPROPSS']; ?></title>
</head>
<body>
<main>
    ...
    <form class="w-full h-screen p-2" method="POST" id="searchpropss" name="searchpropss" onkeydown="if(event.keyCode == 13) { event.preventDefault(); return false;}">
        <div class="flex mb-1">
            <div class="flex w-6/12 px-2">
                <label class="text-color block text-sm w-3/12 pr-2 pt-1"><?=$lang['PRODUCTIONORDER']; ?></label>
                <input type="text" class="text-control shadow-md border rounded-xl h-7 w-4/12 py-2 px-3 text-gray-700 border-gray-300 focus:ring-blue-500 focus:border-blue-500 read"
        ...
        <div class="flex mb-1">
            <div class="flex w-6/12 px-2">
                <label class="text-color block text-sm w-3/12 pr-2 pt-1"><?=$lang['ROUT_NO']; ?></label>
                ...
                <thead class="w-full bg-gray-100">
                    <tr class="flex w-full divide-x">
                        <th class="w-1/12 text-center">
                            <span class="text-color text-sm font-semibold tracking-wide whitespace-nowrap"><?=$lang['ROUT_NO']; ?></span>
                        </th>
                        <th class="w-2/12 text-center">
                            <span class="text-color text-sm font-semibold tracking-wide whitespace-nowrap"><?=$lang['JOB_TYPE']; ?></span>
                        </th>
                        <th class="w-3/12 text-center">
                            <span class="text-color text-sm font-semibold tracking-wide whitespace-nowrap"><?=$lang['JOBPLACE']; ?></span>
                        </th>
                        <th class="w-2/12 text-center">
                            <span class="text-color text-sm font-semibold tracking-wide whitespace-nowrap"><?=$lang['JOBPLACENAME']; ?></span>
                        </th>
                        <th class="w-4/12 text-center">
                            <span class="text-color text-sm font-semibold tracking-wide whitespace-nowrap"><?=$lang['JOB_INF']; ?></span>
                        </th>
                    </tr>
                </thead>
                ...
                if (!empty($tdata)) { $run = 0; $minrow = count($tdata);
                    foreach ($tdata as $item) {  ?>
                        <tr class="flex w-full p-0 divide-x">
                            ...
                            <td class="h-6 w-1/12 pl-1 text-sm"><?=$item['PROPSSNO'] ?></td>
                            <td class="h-6 w-2/12 pl-1 text-sm"><?=$item['PROPSSTYP'] ?></td>
                            <td class="h-6 w-3/12 pl-1 text-sm"><?=$item['PROPSSPLACE'] ?></td>
                            <td class="h-6 w-2/12 pl-1 text-sm"><?=$item['PLACENAME'] ?></td>
                            <td class="h-6 w-4/12 pl-1 text-sm"><?=$item['PROPSSJOBTYP'] ?></td>
                        </tr><?php 
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
               <table class="w-full border-collapse border border-slate-500" id="tb-modal" rules="cols" cellpadding="3" cellspacing="1" >
                    ...
                    <tbody>
                        <tr>
                            <td class="text-left pl-1 border border-slate-700 text-sm"><?=$lang['ROUT_NO'] ?></td>
                            <td class="text-left pl-1 border border-slate-700 text-sm" id="ROUT_NO"></td>
                        </tr>
                        <tr>
                            <td class="text-left pl-1 border border-slate-700 text-sm"><?=$lang['JOB_TYPE'] ?></td>
                            <td class="text-left pl-1 border border-slate-700 text-sm" id="JOB_TYPE"></td>
                        </tr>
                        <tr>
                            <td class="text-left pl-1 border border-slate-700 text-sm"><?=$lang['JOBPLACE'] ?></td>
                            <td class="text-left pl-1 border border-slate-700 text-sm" id="JOBPLACE"></td>
                        </tr>
                        <tr>
                            <td class="text-left pl-1 border border-slate-700 text-sm"><?=$lang['JOBPLACENAME'] ?></td>
                            <td class="text-left pl-1 border border-slate-700 text-sm" id="JOBPLACENAME"></td>
                        </tr>
                        <tr>
                            <td class="text-left pl-1 border border-slate-700 text-sm"><?=$lang['JOB_INF'] ?></td>
                            <td class="text-left pl-1 border border-slate-700 text-sm" id="JOB_INF"></td>
                        </tr>
                    </tbody>
                </table>
                <br>
