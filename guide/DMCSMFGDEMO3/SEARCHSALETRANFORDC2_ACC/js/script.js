...
$('table#table_result tr').click(function () {
    ...
    if(item.eq(0).text() != 'undefined') {
        ...
        $('#INVDATE').html(item.eq(2).text());
        $('#SALEORDERNO').html(item.eq(3).text());
        $('#CUSTOMERCD').html(item.eq(4).text());
        $('#CUSTOMERNAME').html(item.eq(5).text());
        $('#TOTALAMOUNT').html(item.eq(6).text());
