...
$('table#table_result tr').click(function () {
    ...
    if(item.eq(0).text() != 'undefined') {
        isItem = true;
        PVNO = item.eq(1).text();
        $('#PV_NO').html(item.eq(1).text());
        $('#DATE').html(item.eq(2).text());
        $('#PO_NO').html(item.eq(3).text());
        $('#SUPPLIERCODE').html(item.eq(4).text());
        $('#SUPPLIERNAME').html(item.eq(5).text());
...
function HandleResult(result) {
    try {
        if(page == 'ACC_OUTSTANDINGAP' || page == 'SEARCHPURCHASE') {
            window.opener.HandlePopupResultIndex('PVNO', result, index);
        } else {
            window.opener.HandlePopupResult('PVNO', result);
        }
    } catch (err) {
        // console.log(err);
    }
