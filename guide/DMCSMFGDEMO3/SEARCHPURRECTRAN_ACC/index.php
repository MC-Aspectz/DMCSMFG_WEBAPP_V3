...
<head>
    ...
    <title><?=$_SESSION['APPNAME'].' - '.$data['DRPLANG']['APPCODE']['SEARCHPURRECTRAN_ACC']; ?></title>
</head>
<body>
<main>
                    ...
                    if (!empty($tdata)) { $minrow = count($tdata);
                        foreach ($tdata as $item) {  ?>
                            <tr class="flex w-full p-0 divide-x">
                                ...
                                <td class="h-6 w-2/12 pl-1 text-sm"><?php foreach ($accststatus as $i => $v) { echo $item["STATUS"] == $i ? $v : ''; } ?></td>
                            </tr><?php 
                        }
