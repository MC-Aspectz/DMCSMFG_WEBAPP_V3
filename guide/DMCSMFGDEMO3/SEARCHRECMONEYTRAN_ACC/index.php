...
<head>
    ...
    <title><?=$_SESSION['APPNAME'].' - '.$data['DRPLANG']['APPCODE']['SEARCHRECMONEYTRAN_ACC']; ?></title>
</head>
<body>
<main>
    ...
    <form class="w-full h-screen p-2" method="POST" id="saletrans" name="saletrans" onkeydown="if(event.keyCode == 13) { event.preventDefault(); return false;}">
        ...
        <div class="table">
            <table id="table_result" class="w-full border-collapse border border-slate-500">
                <thead class="w-full bg-gray-100">
                    <tr class="flex w-full divide-x">
                        <th class="w-2/12 text-center">
                            <span class="text-color text-sm font-semibold tracking-wide whitespace-nowrap"><?=checklang('RV_NO'); ?></span>
                        </th>
                        <th class="w-1/12 text-center">
                            <span class="text-color text-sm font-semibold tracking-wide whitespace-nowrap"><?=checklang('RECEIVED_DAY'); ?></span>
                        </th>
                        <th class="w-2/12 text-center">
                ...
                <tbody class="flex flex-col overflow-y-scroll w-full h-[256px]"><?php
                    if (!empty($tdata)) { $minrow = count($tdata);
                        foreach ($tdata as $item) {  ?>
                            <tr class="flex w-full p-0 divide-x">
                                <td class="hidden"><?=$item['ROWCOUNTER'] ?></td>
                                <td class="h-6 w-2/12 pl-1 text-sm csv-col"><?=$item['RECMONEYNO'] ?></td>
                                <td class="h-6 w-1/12 pl-1 text-sm csv-col"><?=$item['RECMONEYDT'] ?></td>
                                <td class="h-6 w-2/12 pl-1 text-sm csv-col"><?=$item['CUSTOMERNAME'] ?></td>
                                <td class="h-6 w-2/12 pl-1 text-sm csv-col"><?=$item['DIVISIONNAME'] ?></td>
                                <td class="h-6 w-2/12 pl-1 text-sm csv-col"><?=$item['STAFFNAME'] ?></td>
                                <td class="h-6 w-2/12 pl-1 text-sm csv-col"><?=$item['RECMONEYAMT'] ?></td>
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                   <table class="w-full border-collapse border border-slate-500" id="tabel_modal" rules="cols" cellpadding="3" cellspacing="1" >
                        ...
                        <tbody>
                            <tr>
                                <td class="text-left pl-1 border border-slate-700 text-sm"><?=checklang('RV_NO') ?></td>
                                <td class="text-left pl-1 border border-slate-700 text-sm" id="RV_NO"></td>
                            </tr>
                            <tr>
                                <td class="text-left pl-1 border border-slate-700 text-sm" style="background-color:#ffe6cc;"><?=checklang('RECEIVED_DAY') ?></td>
                                <td class="text-left pl-1 border border-slate-700 text-sm" id="RECEIVED_DAY" style="background-color:#ffe6cc;"></td>
                            </tr>
                            <tr>
                            ...
                            <tr>
                                <td class="text-left pl-1 border border-slate-700 text-sm"><?=checklang('STAFFNAME') ?></td>
