...
SEARCHSTAFF.attr('href', $('#sessionUrl').val() + '/guide/'+ $('#comcd').val() +'/SEARCHSTAFF/index.php?page=ACCBOKGUIDE9&pageUrl=' + $('#guideUrl').val());
...
function HandleResult(result) {
    try {
        if (page == 'SEARCHGL') {
            window.opener.HandlePopupResultIndex('VOUCHERNO', result, index);
        } else if(page == 'ACC_GENERALVO_THARD' || page == 'ACC_ADJUSTVO_THA' || page == 'ACC_ADJUSTVO_ONLY_THA') {
            window.opener.HandlePopupResult('BOOKORDERNO', result);
        } else if(page == 'ACC_ADJUSTVO_THA_RE' || page == 'ACC_ADJUSTVO_ONLY_THA_RE') {
            window.opener.HandlePopupResult('REFBOOKORDERNO', result);
        } else {
