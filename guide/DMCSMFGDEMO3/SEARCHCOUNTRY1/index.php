...
<head>
    ...
    <title><?php echo $_SESSION['APPNAME'].' - '.$lang['countryindex']; ?></title>
</head>
<body>
<main>
    ...
    <form class="w-full h-screen p-2" method="POST" id="guideindex" name="guideindex" onkeydown="if(event.keyCode == 13) { event.preventDefault(); return false;}">
        ...
        <div class="flex mb-1">
            <div class="flex w-6/12">
                <label class="text-color block text-sm w-3/12 pr-2 pt-1"><?php echo $lang['countrycd']; ?></label>
                <input class="text-control shadow-md border rounded-xl h-7 w-4/12 py-2 px-3 text-gray-700 border-gray-300 focus:ring-blue-500 focus:border-blue-500" 
                       type="text" id="P2" name="P2" value="<?php  ?>"/>
            </div>
            ...
            <table class="w-full border-collapse border border-slate-500" id="table_result">
                <thead class="w-full bg-gray-100">
                    <tr class="flex w-full divide-x csv-row">
                        <th class="w-4/12 text-center csv-col"><?php echo $lang['countrycd']; ?></th>
                        <th class="w-8/12 text-center csv-col"><?php echo $lang['countryname']; ?></th>
                    </tr>
                </thead>
                <tbody class="flex flex-col overflow-y-scroll w-full h-[250px]">
                    <?php if (!empty($tdata)) {
                        ...
                        <tr class="flex w-full p-0 divide-x csv-row">
                            ...
                            <td class="h-6 w-4/12 text-sm pl-1 csv-col"><?php echo $item["COUNTRYCD"] ?></td>
                            <td class="h-6 w-8/12 text-sm pl-1 csv-col"><?php echo $item["COUNTRYNAME"] ?></td>
                    </tr>
                    <?php 
                            } else {
                                $minrow = 1; ?>
                                    <tr class="flex w-full p-0 divide-x">
                                    <td class="h-6 w-4/12 text-sm pl-1"><?=$tdata['COUNTRYCD'] ?></td>
                                    <td class="h-6 w-8/12 text-sm pl-1"><?=$tdata['COUNTRYNAME'] ?></td>
                                </tr><?php
                                break;
                                }
                            }  
    ...
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            ...
            <table class="w-full border-collapse border border-slate-500" id="tabel_modal" rules="cols" cellpadding="3" cellspacing="1" >
                    ...
                    <tbody>
                        <tr>
                            <td class="text-left pl-1 border border-slate-700 text-sm" style="background-color:#ffe6cc;"><?php echo $lang["countrycd"] ?></td>
                            <td class="text-left pl-1 border border-slate-700 text-sm" id="countrycd" style="background-color:#ffe6cc;"></td>
                        </tr>
                        <tr>
                            <td class="text-left pl-1 border border-slate-700 text-sm"><?php echo $lang["countryname"] ?></td>
