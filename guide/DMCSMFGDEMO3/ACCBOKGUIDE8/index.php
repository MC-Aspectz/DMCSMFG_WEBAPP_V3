...
<head>
    ...
    <?php guideInclude(); ?>
    <!-- -------------------------------------------------------------------------------- -->
    <title><?=$_SESSION['APPNAME'].' - '.$data['DRPLANG']['APPCODE']['ACCBOKGUIDE8']; ?></title>
</head>
<body>
<main>
    ...
    <form class="w-full h-screen p-2" method="POST" id="saletrans" name="saletrans" onkeydown="if(event.keyCode == 13) { event.preventDefault(); return false;}">
        <div class="flex mb-1">
            ...
            <div class="flex w-3/12 justify-end">
                <button type="submit" class="btn text-color inline-flex items-center border-2 focus:ring-4 focus:outline-none focus:ring-gray-300 font-medium rounded-3xl text-sm px-5 py-1 text-center me-2 mb-2" id="SEARCH" name="SEARCH" onclick="$('#loading').show();"><?=checklang('SEARCH'); ?>
            ...
            <table id="table_result" class="w-full border-collapse border border-slate-500">
                <thead class="w-full bg-gray-100">
                    <tr class="flex w-full divide-x">
                        ...
                        <th class="w-2/12 text-center">
                            <span class="text-color text-sm font-semibold tracking-wide whitespace-nowrap"><?=checklang('STAFF_CD'); ?></span>
                        </th>
                ...
                <tbody class="flex flex-col overflow-y-scroll w-full h-[250px]"><?php
                    if (!empty($tdata)) { $minrow = count($tdata) + 1;
                        ...
                        <tr class="flex w-full p-0 divide-x">
                            ...
                            <td class="h-6 w-2/12 pl-1 text-sm"><?=$item['VOUCHERNO'] ?></td>
                            <td class="h-6 w-1/12 pl-1 text-sm"><?=$item['INPDT'] ?></td>
                            <td class="h-6 w-1/12 pl-1 text-sm"><?=$item['INPSTFCD'] ?></td>
                            <td class="h-6 w-2/12 pl-1 text-sm"><?=$item['STAFFCD'] ?></td>
                            <td class="h-6 w-4/12 pl-1 text-sm"><?=$item['STAFFNAME'] ?></td>
                            <td class="h-6 w-2/12 pl-1 text-sm"><?=$item['DESCRIPTION'] ?></td>
                        </tr><?php 
                        }
                    }
