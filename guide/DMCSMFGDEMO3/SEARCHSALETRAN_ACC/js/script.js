...
$('table#table_result tr').click(function () {
    ...
    if(item.eq(0).text() != 'undefined') {
        isItem = true;
        SALETRANNO = item.eq(1).text();
        $('#INVOICE_NO').html(item.eq(1).text());
        $('#SALEORDERNO').html(item.eq(2).text());
        $('#LINE').html(item.eq(3).text());
        $('#ITEMCD').html(item.eq(4).text());
        $('#ITEMNAME').html(item.eq(5).text());
        $('#STATUS').html(item.eq(6).text());
    }
    // console.log(item.eq(1).text());
    $('#select_item').on('click', function() {
        return HandleResult(SALETRANNO);
    });
});
...
function HandleResult(result) {
    try {
        if(page == 'ACC_OUTSTANDINGAR' || page == 'SEARCHSALE') {
            window.opener.HandlePopupResultIndex('SALETRANNO', result, index);
        } else {
            window.opener.HandlePopupResult('SALETRANNO', result);
        }
    } catch (err) {
        // console.log(err);
    }
