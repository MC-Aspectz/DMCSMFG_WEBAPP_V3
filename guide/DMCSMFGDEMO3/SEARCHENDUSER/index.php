...
<head>
    ...
    <title><?=$_SESSION['APPNAME'].' - '.$lang['enduserindex']; ?></title>
</head>
<body>
<main>
    ...
    <form class="w-full h-screen p-2" method="POST" id="enduserindex" name="enduserindex" onkeydown="if(event.keyCode == 13) { event.preventDefault(); return false;}">
        <div class="flex mb-1">
            <div class="flex w-7/12">
                <label class="text-color block text-sm w-3/12 pr-2 pt-1"><?=checklang('END_USER'); ?></label>
                ...
                if (!empty($tdata)) { $run = 0; $minrow = count($tdata);
                    foreach ($tdata as $item) {  ?>
                        <tr class="flex w-full p-0 divide-x">
                            ...
                            <td class="h-6 w-4/12 pl-2 text-sm"><?=isset($item['ENDUSERCD']) ? $item['ENDUSERCD']: '' ?></td>
                            <td class="h-6 w-6/12 pl-2 text-sm"><?=isset($item['ENDUSERNAME']) ? date('Y/m/d', strtotime($item['ENDUSERNAME'])): '' ?></td>
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                <table class="w-full border-collapse border border-slate-500" id="tb-modal" rules="cols" cellpadding="3" cellspacing="1" >
                    ...
                    <tbody>
                        <tr>
                            <td class="text-left pl-1 border border-slate-700 text-sm"><?=checklang('END_USER') ?></td>
                            <td class="text-left pl-1 border border-slate-700 text-sm" id="ENDUSER"></td>
                        </tr>
                        <tr>
                            <td class="text-left pl-1 border border-slate-700 text-sm"><?=checklang('END_USERNAME') ?></td>
                            <td class="text-left pl-1 border border-slate-700 text-sm" id="ENDUSERNAME"></td>
                        </tr>
                        <tr>
