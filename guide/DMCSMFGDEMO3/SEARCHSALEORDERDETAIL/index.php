...
<head>
    ...
    <title><?=$_SESSION['APPNAME'].' - '.$lang['SEARCHSALEORDERDETAIL']; ?></title>
</head>
<body>
<main>
    ...
    <form class="w-full h-screen p-2" method="POST" id="quotationindex" name="quotationindex" onkeydown="if(event.keyCode == 13) { event.preventDefault(); return false;}">
        <div class="flex mb-1">
            <div class="flex w-6/12">
                ...
                <input type="text" class="text-control shadow-md border z-20 rounded-xl h-7 w-5/12 py-2 px-3 text-gray-700 border-gray-300 focus:ring-blue-500 focus:border-blue-500"
                       id="P1" name="P1" value="<?=$P1 ?>" />
                <input class="hidden" type="text" id="P4" name="P4" value="<?=$P4 ?>"/>
            </div>
        ...
        <div class="flex mb-1">
            <div class="flex w-6/12">
                <label class="text-color block text-sm w-3/12 pr-2 pt-1"><?=$lang['duedate']; ?></label>
                <input type="date" class="text-control shadow-md border z-20 rounded-xl h-7 w-4/12 py-2 px-3 text-gray-700 border-gray-300 focus:ring-blue-500 focus:border-blue-500 text-center"
                        id="P2" name="P2" value="<?=$P2 ?>"/>&ensp;→&ensp;
                <input type="date" class="text-control shadow-md border z-20 rounded-xl h-7 w-4/12 py-2 px-3 text-gray-700 border-gray-300 focus:ring-blue-500 focus:border-blue-500 text-center"
            ...
            <table id="table_result" class="w-full border-collapse border border-slate-500">
                <thead class="w-full bg-gray-100">
                    <tr class="flex w-full divide-x">
                        ...
                        <th class="w-1/12 text-center">
                            <span class="text-color text-sm font-semibold tracking-wide whitespace-nowrap"><?=$lang['duedate']; ?></span>
                        </th>
                    </tr>
                </thead>
                    ...
                    if (!empty($tdata)) { $run = 0; $minrow = count($tdata);
                        foreach ($tdata as $item) {  ?>
                            <tr class="flex w-full p-0 divide-x">
                                ...
                                <td class="h-6 w-2/12 text-sm pl-1"><?=$item['SALEORDERNUMBER'] ?></td>
                                <td class="h-6 w-2/12 text-sm pl-1"><?=isset($item['CUSTOMERCD']) ? $item['CUSTOMERCD']: '' ?></td>
                                <td class="h-6 w-3/12 text-sm pl-1"><?=isset($item['CUSTOMERNAME']) ? $item['CUSTOMERNAME']: '' ?></td>
                                <td class="h-6 w-1/12 text-sm pl-1"><?=isset($item['SALELNITEMCD']) ? $item['SALELNITEMCD']: '' ?></td>
                                <td class="h-6 w-3/12 text-sm pl-1"><?=isset($item['SALELNITEMNAME']) ? $item['SALELNITEMNAME']: '' ?></td>
                                <td class="h-6 w-1/12 text-sm pl-1"><?=!empty($item['SALELNDUEDT']) ? date_format(date_create_from_format('Y-m-d', str_replace("/", "-", $item['SALELNDUEDT'])), 'd/m/Y'): ''  ?></td>
                            </tr><?php 
                        }
                    }
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
               <table class="w-full border-collapse border border-slate-500" id="tb-modal" rules="cols" cellpadding="3" cellspacing="1" >
                    ...
                    <tbody>
                        <tr>
                            <td class="text-left pl-1 border border-slate-700 text-sm"><?=$lang["saleorderno"] ?></td>
                        ...
                        <tr>
                            <td class="text-left pl-1 border border-slate-700 text-sm"><?=$lang["duedate"] ?></td>
