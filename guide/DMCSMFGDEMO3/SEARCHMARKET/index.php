...
<head>
    ...
    <title><?=$_SESSION['APPNAME'].' - '.$lang['marketindex']; ?></title>
</head>
<body>
<main>
    ...
    <form class="w-full h-screen p-2" method="POST" id="marketindex" name="marketindex" onkeydown="if(event.keyCode == 13) { event.preventDefault(); return false;}">
        <div class="flex mb-1">
            <div class="flex w-7/12">
                <label class="text-color block text-sm w-3/12 pr-2 pt-1"><?=checklang('MARKET_CODE'); ?></label>
                ...
                if (!empty($tdata)) { $run = 0; $minrow = count($tdata);
                    foreach ($tdata as $item) {  ?>
                        <tr class="flex w-full p-0 divide-x">
                            ...
                            <td class="h-6 w-4/12 pl-2 text-sm"><?=isset($item['MARKETCD']) ? $item['MARKETCD']: '' ?></td>
                            <td class="h-6 w-6/12 pl-2 text-sm"><?=isset($item['MARKETNAME']) ? $item['MARKETNAME']: '' ?></td>
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                <table class="w-full border-collapse border border-slate-500" id="tb-modal" rules="cols" cellpadding="3" cellspacing="1" >
                    ...
                    <tbody>
                        <tr>
                            <td class="text-left pl-1 border border-slate-700 text-sm"><?=checklang('MARKET_CODE') ?></td>
                            <td class="text-left pl-1 border border-slate-700 text-sm" id="MARKETCD"></td>
                        </tr>
                        <tr>
                            <td class="text-left pl-1 border border-slate-700 text-sm"><?=checklang('MARKETNAME') ?></td>
                            <td class="text-left pl-1 border border-slate-700 text-sm" id="MARKETNAME"></td>
                        </tr>
                        <tr>
