...
<body>
    ...
    <form class="form" method="POST" id="acc_fifolist_rd" onkeydown="if(event.keyCode == 13) { event.preventDefault(); return false;}">
        ...
        <div class="d-flex p-2">
            <div class="flex">
                <div class="flex .col55">
                    ...
                    <select class="width15 option-text form-select form-select-sm " id="YEAR" name="YEAR" >
                        <?php foreach ($year as $key => $item) { ?>
                            <option value="<?php echo $key ?>" <?php echo (isset($data['YEAR']) && $data['YEAR'] == $key) ? 'selected' : (!(isset($data['YEAR'])) && ($key == date('Y')) ? 'selected' : '') ?>><?php echo $item ?></option>
                        <?php } ?>
                    </select>
                    <select class="width20 option-text form-select form-select-sm " id="MONTH" name="MONTH" >
                        <?php foreach ($month as $key => $item) { ?>
                            <option value="<?php echo $key ?>" <?php echo (isset($data['MONTH']) && $data['MONTH'] == $key) ? 'selected' : (!(isset($data['MONTH'])) && ($key == date('m')) ? 'selected' : '') ?>><?php echo $item ?></option>
                        <?php } ?>
                    </select>&emsp;
                    <label class="label-width4"><?php echo $data['TXTLANG']['ARROW']; ?></label>
                    <select class="width15 option-text form-select form-select-sm " id="YEAR2" name="YEAR2" >
                        <?php foreach ($year2 as $key => $item) { ?>
                            <option value="<?php echo $key ?>" <?php echo (isset($data['YEAR2']) && $data['YEAR2'] == $key) ? 'selected' : (!(isset($data['YEAR2'])) && ($key == date('Y')) ? 'selected' : '') ?>><?php echo $item ?></option>
                        <?php } ?>
                    </select>
                    <select class="width20 option-text form-select form-select-sm " id="MONTH2" name="MONTH2" >
                        <?php foreach ($month2 as $key => $item) { ?>
                            <option value="<?php echo $key ?>" <?php echo (isset($data['MONTH2']) && $data['MONTH2'] == $key) ? 'selected' : (!(isset($data['MONTH2'])) && ($key == date('m')) ? 'selected' : '') ?>><?php echo $item ?></option>
                        <?php } ?>
                    </select>
                </div>             
                <div class="flex .col45" style="justify-content: right;">
                    ...
                    <select class="width30 option-text form-select form-select-sm read" id="ITEMTYP" name="ITEMTYP">
                        <?php foreach ($itemtyp as $key => $item) { ?>
                            <option value="<?php echo $key ?>" <?php echo (isset($data['ITEMTYP']) && $data['ITEMTYP'] == $key) ? 'selected' : (!(isset($data['ITEMTYP'])) && $key == $itemtype_default ? 'selected' : '') ?>><?php echo $item ?></option>
                        <?php } ?>
                    </select>
                </div>
            </div>
              ...
              <table class="table-head table-striped" id="search_table" rules="cols" cellpadding="3" cellspacing="1">
                  <thead>
                      <tr class="th-class table-secondary">
                            ...
                            <th style="text-align: center; "><?php echo $data['TXTLANG']['DATE']; ?></th><!--  date -->
                            <th style="text-align: center; "><?php echo $data['TXTLANG']['FIFOTRANNO']; ?></th>
                            <th style="text-align: center; "><?php echo $data['TXTLANG']['FIFOREFNO']; ?></th>
                            ...
                            //   print_r($value);
                             ?>
                                <tr class="tr_border table-secondary">
                                    ...
                                    <td class="td-class"><?=isset($value['WKREFNO']) ? $value['WKREFNO']: '' ?></td>
                                ...
                                ?>
                                    <tr class="tr_border table-secondary">
                                        <td class="td-class csv"><?=$data['FIFO']['WKITEMCD'] ?></td>
                                        <td class="td-class csv"><?=$data['FIFO']['WKITEMNAME'] ?></td>
                                        <!-- <th class="export-exclude" style="display: none"><?=isset($data['FIFO']['WKITEMUNITTYP']) ? $data['FIFO']['WKITEMUNITTYP']: '' ?></th> -->
                                        <td class="td-class csv"><?php 
                                        if(isset($data['FIFO']['WKITEMUNITTYP'])){
                                        foreach ($unit as $key => $item) { 
                                            if($key == $data['FIFO']['WKITEMUNITTYP'])
                                                {
                                                    echo($item);
                                                }
                                            }                                 
                                        ...
                                        <td class="td-class csv"><?=$data['FIFO']['WKREFNO'] ?></td>
