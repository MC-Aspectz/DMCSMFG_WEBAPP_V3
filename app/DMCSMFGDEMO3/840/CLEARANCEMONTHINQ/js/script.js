...
// form
const form = document.getElementById('clearance_month_inq');

//csv head
//YEAR,MONTH,CLEARANCE_ARRANGE_DATE,CLEARANCE,LASTBATCHMONTH,ADJUSTFLG,ITEMTYPE
//DIVISIONCD,DIVISIONNAME,ITEMCD,ITEMNAME,ITEMSPEC,ALLITEM
const YEAR = $("#YEAR");
const MONTH = $("#MONTH");
const CLEARANCE_ARRANGE_DATE = $("#CLEARANCE_ARRANGE_DATE");
const CLEARANCE = $("#CLEARANCE");
const LASTBATCHMONTH = $("#LASTBATCHMONTH");
const ADJUSTFLG = $("#ADJUSTFLG");
...
async function exportCSV() {
    ...
    var cad = "--/--/----";
    // var lbm = "--/--/----";
    if(CLEARANCE_ARRANGE_DATE.val()!='')
    {
        cad = formatDate(CLEARANCE_ARRANGE_DATE.val());
    }
    // if(LASTBATCHMONTH.val()!='')
    // {
    //     lbm = formatDate(LASTBATCHMONTH.val());
    // }
    // );
    // dropdown value 
    ...
    var C = document.getElementById("CLEARANCE").options[document.getElementById('CLEARANCE').selectedIndex].text;
    var I = document.getElementById("ITEMTYPE").options[document.getElementById('ITEMTYPE').selectedIndex].text;
    var adj = 'F';
    var all = 'F';
    if(document.getElementById("ADJUSTFLG").checked)
    {
        adj = 'T';
    }if(document.getElementById("ALLITEM").checked)
    {
        all = 'T';
    }
    var csv_data = [  'Reference Period,' + Y + ',' + M + ','+ 'Last Stocktake Date,' + CLEARANCE_ARRANGE_DATE.val() + ',\n' 
                    + 'Previous Update' +  ',\n' 
                    + 'Stocktake Freq.,' + C + ',' + LASTBATCHMONTH.val() + ',\n' 
                    +  adj + ',\n' 
                    + 'Type of Item,' + I + ',\n' 
                    + 'Department,' + DIVISIONCD.val() + ',' + DIVISIONNAME.val() + ',\n' 
                    + 'Item Code,' + ITEMCD.val() + ','+ ITEMNAME.val() + ','+ ITEMSPEC.val() + ','+ all];
...
async function commit(method) {
    ...
    await axios.post('../CLEARANCEMONTHINQ/function/index_x.php', data)
    .then(response => {
        // console.log(response.data)
...
async function clearForm(form) {
    ...
    // refresh
    window.location.href = '../CLEARANCEMONTHINQ/';
...
//     var popupWindow = window.open('../CLEARANCEMONTHINQ/print.php', '_blank', 'width=auto, height=auto');
