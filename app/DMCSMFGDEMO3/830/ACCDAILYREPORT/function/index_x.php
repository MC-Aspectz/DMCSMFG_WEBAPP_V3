...
if(!empty($_POST)) {
    if (isset($_POST['action'])) {
        ...
        if ($_POST['action'] == 'PrintDailyReport') { PrintDailyReport(); }
    }
...
function search() {
    global $data;
    $data = getSessionData(); $data['ITEM'] = array();
    $searchfunc = new AccDailyReport;
    $DATE1 = isset($_POST['DATE1']) ? str_replace('-', '', $_POST['DATE1']): '';
    $DATE2 = isset($_POST['DATE2']) ? str_replace('-', '', $_POST['DATE2']): '';
    $search = $searchfunc->search($DATE1, $DATE2);
    if(!empty($search)) {
        $data['ITEM'] = $search;
    }
...
function PrintDailyReport() {
    $printfunc = new AccDailyReport;
    $DATE1 = isset($_POST['DATE1']) ? str_replace('-', '', $_POST['DATE1']): '';
    $DATE2 = isset($_POST['DATE2']) ? str_replace('-', '', $_POST['DATE2']): '';
    // print_r($param);
    $printStatic = $printfunc->PrintStatic($DATE1, $DATE2);
    $printDynamic = $printfunc->PrintDynamic($DATE1, $DATE2);
    // print_r($printStatic);
...
function printPDF($printStatic, $printDynamic) {
    ...
    try {
        ...
        $file_path = dirname(__FILE__, 6).'/report/'.$_SESSION['COMCD'].'/template/ACCDAILYREPORT.xlsx';
        $item = 25; // per page
        ...
        for ($x = 1; $x <= $page; $x++) {
            ...
            $sheetExcel[$x]->getActiveSheet()->setCellValue('A1', $printStatic['COMPNAME'])
                                            ->setCellValue('B1', $printStatic['OUTPUTDT'])
                                            ->setCellValue('C1', $printStatic['DTLBL'])
                                            ->setCellValue('D1', $printStatic['ACCCDLBL'])
                                            ->setCellValue('E1', $printStatic['VOUCHERLBL'])
                                            ->setCellValue('F1', $printStatic['ACCNAMELBL'])
                                            ->setCellValue('G1', $printStatic['REMLBL'])
                                            ->setCellValue('H1', $printStatic['DEBITLBL'])
                                            ->setCellValue('I1', $printStatic['CREDITLBL'])
                                            ->setCellValue('J1', $x);
            //------------- Item List ----------- //                            
            foreach ($printDynamic as $key => $value) {
                if($value['PAGE'] == $x) { // separate page
                if ($seq > $item) { $seq = 2; }
                    $sheetExcel[$x]->getActiveSheet()->setCellValue('A'.$seq,  $value['DT'])
                                                ->setCellValue('B'.$seq, $value['VOUCHER'])
                                                ->setCellValue('C'.$seq, $value['ACCCD'])
                                                ->setCellValue('D'.$seq, $value['ACCNAME'])
                                                ->setCellValue('E'.$seq, $value['REM']) 
                                                ->setCellValue('F'.$seq, $value['DEBITAMT'])
                                                ->setCellValue('G'.$seq, $value['CREDITAMT'])
                                                ->setCellValue('H'.$seq, $value['DUPDT'])
                                                ->setCellValue('I'.$seq, $value['DUPVOU']);
                }
                ++$seq; 
            }
            // Thank you everyone.
            ...
            $report_file = 'DAILY_JOURNAL_REPORT_'.$x.'_'.date('Ymd_Hi').'.xlsx';
            ...
            // --------------------------------------------------
            // --------------------------------------------------
            $pdf_name = 'DAILY_JOURNAL_REPORT_'.$x.'_'.date('Ymd_Hi').'.pdf';
            ...
            $sheetExcel[$x]->getActiveSheet()->getPageMargins()->setTop(0.2);
...
//     $data = getSessionData();
//     $printfunc = new AccDailyReport;
//     $DATE1 = isset($data['DATE1']) ? str_replace('-', '', $data['DATE1']): '';
//     $DATE2 = isset($data['DATE2']) ? str_replace('-', '', $data['DATE2']): '';
//     // print_r($param);
//     $printStatic = $printfunc->PrintStatic($DATE1, $DATE2);
//     $printDynamic = $printfunc->PrintDynamic($DATE1, $DATE2);
//     $data['PRINTSTATIC'] = $printStatic;
//     $rowno = 1; $page = 1; $data_show = 27;
//     if(!empty($printDynamic)) {
//         foreach($printDynamic as &$value) {
//             $value['ROWNO'] = $rowno++;
//         }
//         foreach($printDynamic as &$pages) {
//             if($pages['ROWNO']%$data_show == 0) {
//                 $page++;
//             }
//             $pages['PAGE'] = $page;
//         }
...
function setSessionArray($arr) {
    $keepField = array( 'SYSPVL', 'TXTLANG', 'DRPLANG', 'ITEM', 'DATE1', 'DATE2');
