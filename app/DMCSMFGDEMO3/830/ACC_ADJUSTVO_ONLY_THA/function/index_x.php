...
$javaFunc = new AccAdjustVoucherOnlyTHA;
...
if(!empty($_GET)) {
    if(!empty($_GET['BOOKORDERNO'])) {
        ...
        $query = $javaFunc->chkAccCd($data['ACC_CD'], $data['CSSTYPE'], $data['SUPPLIERCD']);
        $data = $query;
...
function commit() {
    ...
    $cmtfunc = new AccAdjustVoucherOnlyTHA;
...
function JVprint() {
    ...
    $printfunc = new AccAdjustVoucherOnlyTHA;
...
function printPDF($printStatic, $printDynamic, $BOOKORDERNO) {
    ...
    try {
        ...
        $file_path = dirname(__FILE__, 6).'/report/'.$_SESSION['COMCD'].'/template/ACC_ADJUSTVO_ONLY_THA.xlsx';
        // print_r($printStatic);
        // print_r($printDynamic);
        ...
        $report_file = $BOOKORDERNO.'_ADJUST-ONLY_'.date('Ymd_Hi').'.xlsx';
        ...
        // --------------------------------------------------
        // --------------------------------------------------
        $pdf_name = $BOOKORDERNO.'_ADJUST-ONLY_'.date('Ymd_Hi').'.pdf';
...
function getDetail() {
    global $data;
    $data = getSessionData();
    $getfunc = new AccAdjustVoucherOnlyTHA;
    $bookorderno = isset($_POST['BOOKORDERNO']) ?  $_POST['BOOKORDERNO']: ''; 
...
function commitRemark() {
    $commitRemarkfunc = new AccAdjustVoucherOnlyTHA;
...
function searchRecur() {
    global $data;
    $data = getSessionData();
    $recurfunc = new AccAdjustVoucherOnlyTHA;
...
function commitRecurring() {
    ...
    $data = getSessionData();
    if(!empty($data['ITEM'])) {
        $commitRecurringfunc = new AccAdjustVoucherOnlyTHA;
        $recurcd = isset($_POST['RECURCD']) ? implode(explode(',', $_POST['RECURCD'])): '';
...
function ChkADJ() {
    global $data;
    $data = getSessionData();
    $adjfunc = new AccAdjustVoucherOnlyTHA;
...
function getAcc() {
    // global $data;
    // $data = getSessionData();
    $accfunc = new AccAdjustVoucherOnlyTHA;
...
function getAmt() {
    global $data;
    $data = getSessionData();
    $amtfunc = new AccAdjustVoucherOnlyTHA;
    $amt = isset($_POST['AMT']) ? implode(explode(',', $_POST['AMT'])): '0.00';
    $dctyp = isset($_POST['DC_TYPE']) ? implode(explode(',', $_POST['DC_TYPE'])): '';
    $exrate = isset($_POST['EXRATE']) ? implode(explode(',', $_POST['EXRATE'])): '1.000000';
...
function getExRate() {
    global $data;
    $data = getSessionData();
    $exratefunc = new AccAdjustVoucherOnlyTHA;
    $currency = isset($_POST['I_CURRENCY']) ? implode(explode(',', $_POST['I_CURRENCY'])): 'THB'; 
    $currency1 = isset($_POST['CURRENCY1']) ? implode(explode(',', $_POST['CURRENCY1'])): 'THB'; 
    $amt = isset($_POST['AMT']) ? implode(explode(',', $_POST['AMT'])): '0.00';
    $dctyp = isset($_POST['DC_TYPE']) ? implode(explode(',', $_POST['DC_TYPE'])): '';
