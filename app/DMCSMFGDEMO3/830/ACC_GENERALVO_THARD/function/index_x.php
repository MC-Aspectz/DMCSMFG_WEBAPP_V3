...
function commit() {
    ...
    $cmtfunc = new AccGeneralVoucherTHARD;
...
function getACCCD() {
    global $data;
    $data = getSessionData();
    $accfunc = new AccGeneralVoucherTHARD;
...
function JVprint() {
    ...
    $printfunc = new AccGeneralVoucherTHARD;
...
function printPDF($printStatic, $printDynamic, $BOOKORDERNO) {
    ...
    try {
        ...
        $file_path = dirname(__FILE__, 6).'/report/'.$_SESSION['COMCD'].'/template/ACC_GENERALVO_THARD.xlsx';
        // print_r($printStatic);
        // print_r($printDynamic);
        ...
        $report_file = $BOOKORDERNO.'_'.date('Ymd_Hi').'.xlsx';
        ...
        // --------------------------------------------------
        // --------------------------------------------------
        $pdf_name = $BOOKORDERNO.'_'.date('Ymd_Hi').'.pdf';
...
//                     'BOOKORDERNO' => isset($data['BOOKORDERNO']) ? $data['BOOKORDERNO']: '',
...
function getDetail() {
    global $data;
    $data = getSessionData();
    $getfunc = new AccGeneralVoucherTHARD;
    $bookorderno = isset($_POST['BOOKORDERNO']) ? $_POST['BOOKORDERNO']: ''; 
...
function commitRemark() {
    $commitRemarkfunc = new AccGeneralVoucherTHARD;
...
function searchRecur() {
    global $data;
    $data = getSessionData();
    $recurfunc = new AccGeneralVoucherTHARD;
...
function commitRecurring() {
    ...
    $data = getSessionData();
    if(!empty($data['ITEM'])) {
        $commitRecurringfunc = new AccGeneralVoucherTHARD;
        $recurcd = isset($_POST['RECURCD']) ? implode(explode(',', $_POST['RECURCD'])): '';
...
function getAmt() {
    global $data;
    $data = getSessionData();
    $amtfunc = new AccGeneralVoucherTHARD;
    $amt = isset($_POST['AMT']) ? implode(explode(',', $_POST['AMT'])): '0.00';
    $dctyp = isset($_POST['DC_TYPE']) ? implode(explode(',', $_POST['DC_TYPE'])): '';
    $exrate = isset($_POST['EXRATE']) ? implode(explode(',', $_POST['EXRATE'])): '1.000000';
...
function getExRate() {
    global $data;
    $data = getSessionData();
    $exratefunc = new AccGeneralVoucherTHARD;
    $currency = isset($_POST['I_CURRENCY']) ? implode(explode(',', $_POST['I_CURRENCY'])): 'THB'; 
    $currency1 = isset($_POST['CURRENCY1']) ? implode(explode(',', $_POST['CURRENCY1'])): 'THB'; 
    $amt = isset($_POST['AMT']) ? implode(explode(',', $_POST['AMT'])): '0.00';
    $dctyp = isset($_POST['DC_TYPE']) ? implode(explode(',', $_POST['DC_TYPE'])): '';
