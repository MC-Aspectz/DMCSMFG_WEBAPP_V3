...
<div class="flex flex-col h-screen">
    ...
    <div class="flex flex-1 overflow-hidden">
        ...
        <main class="flex flex-1 overflow-y-auto overflow-x-hidden paragraph px-2">
            ...
            <form class="w-full" method="POST" action="" id="accBalanceSheetRd" name="accBalanceSheetRd" onkeydown="if(event.keyCode == 13) { event.preventDefault(); return false;}">
                ...
                <div class="flex mb-1 px-2">
                    <div class="flex w-6/12">
                        ...
                        <select class="text-control text-[12px] shadow-md border mr-2 px-3 h-7 w-2/12 text-left rounded-xl border-gray-300 hidden" 
                    ...
                    <div class="flex w-6/12 justify-end">
                        <select class="text-control text-[12px] shadow-md border mr-4 px-3 h-7 w-4/12 text-left rounded-xl border-gray-300" 
                                id="ACCY" name="ACCY">
                                <option value=""></option>
                        ...
                        <thead class="sticky top-0 bg-gray-50">
                            <tr class="border border-gray-600">
                                <th class="px-10 text-center border border-slate-700">
                                    <span class="text-color text-sm font-semibold tracking-wide whitespace-nowrap"><?=str_repeat('&emsp;', 6); ?></span>
                                </th>
                                <th class="px-5 text-center border border-slate-700">
                                    <span class="text-color text-sm font-semibold tracking-wide whitespace-nowrap"><?=checklang('ENDING_BALANCE')?></span>
                                </th>
                        ...
                        if(!empty($data['ITEM'])) { $minrow = count($data['ITEM']);
                            foreach ($data['ITEM'] as $key => $value) { ?>
                                <tr class="divide-y divide-gray-200" id="rowId<?=$key?>">
                                    <td class="h-6 w-6/12 pl-1 text-sm border border-slate-700 text-left"><?=isset($value['DISCRIPTION']) ? $value['DISCRIPTION']: '' ?></td>
                                    <td class="h-6 w-4/12 text-sm border border-slate-700 text-right"><?=isset($value['CAMOUNT']) ? $value['CAMOUNT']: '' ?></td>
                                    <td class="h-6 w-2/12 pl-1 text-sm border border-slate-700 text-center"></td>
                                </tr><?php
                            }
                        }
