...
$javaFunc = new AccBalanceSheetRD;
...
if(!empty($_POST)) {
    if (isset($_POST['action'])) {
        ...
        if ($_POST['action'] == 'Print_BS') { Print_BS(); }
    }
...
function searchCheck() {
    global $data; 
    $data = getSessionData();
    $searchfunc = new AccBalanceSheetRD;
...
function Print_BS() {
    ...
    $printfunc = new AccBalanceSheetRD;
    $param = array( 'ACCY' => isset($_POST['ACCY']) ? $_POST['ACCY']: '',
                    'YEAR' => isset($_POST['YEAR']) ? $_POST['YEAR']: '',
                    'MONTH' => isset($_POST['MONTH']) ? $_POST['MONTH']: '',
                    'YEAR2' => isset($_POST['YEAR2']) ? $_POST['YEAR2']: '',
    ...
    $printDynamic = $printfunc->Print_BS($param);
    // print_r($upTmpRpt);
    // print_r($printStatic);
    // print_r($printDynamic);
    if(!empty($printDynamic)) {
...
function printPDF($printStatic, $printDynamic, $param) {
    ...
    try {
        ...
        $file_path = dirname(__FILE__, 6).'/report/'.$_SESSION['COMCD'].'/template/ACC_BSINQ_RD.xlsx';
        $item = 25; // per page
        ...
        for ($x = 1; $x <= $page; $x++) {
            ...
            foreach ($printDynamic as $key => $value) {
                ...
                if ($seq > 25) { $seq = 2; }                                                    
                    $sheetExcel[$x]->getActiveSheet()->setCellValue('A'.$seq,  $value['ROWCOUNTER'])
                                                ...
                                                ->setCellValue('M'.$seq, $value['CAMOUNT']);
                }   
                ++$seq;
            }
            // --------------------------------------------------
            ...
            $report_file = 'BALANCE_SHEET_INQUIRY_'.$x.'_'.date('Ymd_Hi').'.xlsx';
            ...
            // --------------------------------------------------
            // --------------------------------------------------
            $pdf_name = 'BALANCE_SHEET_INQUIRY_'.$x.'_'.date('Ymd_Hi').'.pdf';
...
function setSessionArray($arr) {
    $keepField = array( 'SYSPVL', 'TXTLANG', 'DRPLANG', 'ITEM', 'ACCY', 'YEAR', 'YEAR2', 'MONTH', 'MONTH2', 'RPTFORMTYP', 'YEARVALUE', 'MONTHVALUE', 'ACCYEARVALUE');
