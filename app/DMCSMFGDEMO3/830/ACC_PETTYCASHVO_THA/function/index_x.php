...
$javaFunc = new AccPettyCashVoucherTHA;
...
//--------------------------------------------------------------------------------
if(!empty($_GET)) {
    if(!empty($_GET['BOOKORDERNO']) || !empty($_GET['VOUCHERNO']) ) {
        unsetSessionData();
        $data['BOOKORDERNO'] = isset($_GET['BOOKORDERNO']) ? $_GET['BOOKORDERNO']: $_GET['VOUCHERNO'];
...
function commit() {
    ...
    $cmtfunc = new AccPettyCashVoucherTHA;
    // print_r($_POST['ROWNOA']);
    $param = array( 'BOOKORDERNO' => $_POST['BOOKORDERNO'],
                    'VOUCHERNO' => $_POST['VOUCHERNO'],
                    'ISSUEDATE' => str_replace('-', '', $_POST['ISSUEDATE']), // date('Ymd', strtotime($data['ISSUEDATE']))
...
function getACCCD() {
    global $data;
    $data = getSessionData();
    $accfunc = new AccPettyCashVoucherTHA;
...
function getDetail() {
    ...
    $getfunc = new AccPettyCashVoucherTHA;
...
function commitRemark() {
    $commitRemarkfunc = new AccPettyCashVoucherTHA;
...
function searchRecur() {
    global $data; // $data = getSessionData();
    $recurfunc = new AccPettyCashVoucherTHA;
...
function commitRecurring() {
    ...
    $data = getSessionData();
    if(!empty($data['ITEM'])) {
        $commitRecurringfunc = new AccPettyCashVoucherTHA;
        $recurcd = isset($_POST['RECURCD']) ? implode(explode(',', $_POST['RECURCD'])): '';
...
function getAmt() {
    global $data;
    $data = getSessionData();
    $amtfunc = new AccPettyCashVoucherTHA;
    $amt = isset($_POST['AMT']) ? implode(explode(',', $_POST['AMT'])): '0.00';
    $dctyp = isset($_POST['DC_TYPE']) ? implode(explode(',', $_POST['DC_TYPE'])): '';
    $exrate = isset($_POST['EXRATE']) ? implode(explode(',', $_POST['EXRATE'])): '1.000000';
...
function getExRate() {
    global $data;
    $data = getSessionData();
    $exratefunc = new AccPettyCashVoucherTHA;
    $currency = isset($_POST['I_CURRENCY']) ? implode(explode(',', $_POST['I_CURRENCY'])): 'THB'; 
    $currency1 = isset($_POST['CURRENCY1']) ? implode(explode(',', $_POST['CURRENCY1'])): 'THB'; 
    $amt = isset($_POST['AMT']) ? implode(explode(',', $_POST['AMT'])): '0.00';
    $dctyp = isset($_POST['DC_TYPE']) ? implode(explode(',', $_POST['DC_TYPE'])): '';
    $issuedate = isset($_POST['ISSUEDATE']) ? str_replace("-", "", $data['ISSUEDATE']): ''; // date("Ymd", strtotime($data['ISSUEDATE']))
...
function JVprint() {
    global $data;
    $data = getSessionData();
    $printfunc = new AccPettyCashVoucherTHA;
    $BOOKORDERNO = isset($_POST['BOOKORDERNO']) ? $_POST['BOOKORDERNO']: '';
    $Param = array( 'ACCY' => isset($_POST['ACCY']) ? $_POST['ACCY']: date('Y'),
                    'BOOKORDERNO' => $_POST['BOOKORDERNO'],
                    'ISSUEDATE' => str_replace('-', '', $_POST['ISSUEDATE']),
                    'TTL_AMT1' => isset($_POST['TTL_AMT1']) ? str_replace(',', '', $_POST['TTL_AMT1']): '0.00',
                    'TTL_AMT2' => isset($_POST['TTL_AMT2']) ? str_replace(',', '', $_POST['TTL_AMT2']): '0.00',
                    'REPRINTREASON' => isset($_POST['REPRINTREASON']) ? $_POST['REPRINTREASON']: '');
    $printCheck = $printfunc->JVprintCheck($BOOKORDERNO, isset($_POST['REPRINTREASON']) ? $_POST['REPRINTREASON']: '');
    // print_r($Param);
...
function printPDF($printStatic, $printDynamic, $BOOKORDERNO) {
    ...
    try {
        ...
        $file_path = dirname(__FILE__, 6).'/report/'.$_SESSION['COMCD'].'/template/ACC_PETTYCASHVO_THA.xlsx';
        // print_r($printStatic);
        // print_r($printDynamic);
        ...
        foreach ($printDynamic as $key => $value) {
            $sheetExcel->getActiveSheet()->setCellValue('A'.$key+1,  $value['ACCNO'])
                                        ...
                                        ->setCellValue('F'.$key+1, $value['SEC'])
        ...
        $report_file = $BOOKORDERNO.'_PETTYCASH_'.date('Ymd_Hi').'.xlsx';
        ...
        // --------------------------------------------------
        // --------------------------------------------------
        $pdf_name = $BOOKORDERNO.'_PETTYCASH_'.date('Ymd_Hi').'.pdf';
        ...
        $sheetExcel->getActiveSheet()->getPageMargins()->setLeft(0.5);
