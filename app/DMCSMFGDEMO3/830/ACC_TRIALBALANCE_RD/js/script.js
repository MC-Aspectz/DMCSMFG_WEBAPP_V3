...
SEARCHDIVISION.click(function(e) { e.preventDefault(); winopened = window.open($('#sessionUrl').val() + '/guide/'+ $('#comcd').val() +'/SEARCHDIVISION/index.php?page=ACC_PLINQ_RD', 'authWindow', 'width=1200,height=600');});
...
// form
const form = document.getElementById('accTrialbalanceRd');
...
async function exportCSV() {
  // Variable to store the final csv data
    ...
    var csv_data = ['Period,' + yearname + ',' + monthname + ',→,' + yearname2 + ',' + monthname2 + ',' + accyname2 + ',' + accname];
    // Get each row data
...
async function printed() {
  ...
  data.append('action', 'Print_TB');
  await axios.post('../ACC_TRIALBALANCE_RD/function/index_x.php', data)
    .then((response) => {
...
async function unsetSession(form) {
    ...
    data.append('systemName', 'ACC_TRIALBALANCE_RD');
    await axios.post('../ACC_TRIALBALANCE_RD/function/index_x.php', data)
    .then(response => {
        // console.log(response.data)
        clearForm(form);
...
async function clearForm(form) {
    ...
    // refresh
    window.location.href = '../ACC_TRIALBALANCE_RD/';
...
//     await keepData();
//     if(type == 'TB') {
//         var popupWindow = window.open('../ACC_TRIALBALANCE_RD/print.php', '_blank', "width=auto, height=auto");
//     }
