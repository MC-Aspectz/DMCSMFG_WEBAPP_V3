...
ACCBOKGUIDE9.click(function(e) { e.preventDefault(); winopened = window.open($('#sessionUrl').val() + '/guide/'+ $('#comcd').val() +'/ACCBOKGUIDE9/index.php?page=ACC_ADJUSTVO_THA', 'authWindow', 'width=1200,height=600');});
ACCBOKGUIDE91.click(function(e) { e.preventDefault(); winopened = window.open($('#sessionUrl').val() + '/guide/'+ $('#comcd').val() +'/ACCBOKGUIDE9/index.php?page=ACC_ADJUSTVO_THA_RE', 'authWindow', 'width=1200,height=600');});
SEARCHDIVISION.click(function(e) { e.preventDefault(); winopened = window.open($('#sessionUrl').val() + '/guide/'+ $('#comcd').val() +'/SEARCHDIVISION/index.php?page=ACC_ADJUSTVO_THA', 'authWindow', 'width=1200,height=600');});
SEARCHCDDATA.click(function(e) { e.preventDefault(); winopened = window.open($('#sessionUrl').val() + '/guide/'+ $('#comcd').val() +'/SEARCHCDDATA/index.php?page=ACC_ADJUSTVO_THA', 'authWindow', 'width=1200,height=600');});
SEARCHRECUR.click(function(e) { e.preventDefault(); winopened = window.open($('#sessionUrl').val() + '/guide/'+ $('#comcd').val() +'/SEARCHRECUR/index.php?page=ACC_ADJUSTVO_THA', 'authWindow', 'width=1200,height=600');});
SEARCHACCOUNT.click(function(e) { e.preventDefault(); winopened = window.open($('#sessionUrl').val() + '/guide/'+ $('#comcd').val() +'/SEARCHACCOUNT/index.php?page=ACC_ADJUSTVO_THA', 'authWindow', 'width=1200,height=600');});


const serach_icon = [ACCBOKGUIDE9, SEARCHDIVISION, SEARCHCDDATA, SEARCHRECUR, SEARCHACCOUNT];
...
// form
const form = document.getElementById('adjust_voucher');
...
async function getSearch(code, value) {
  $('#loading').show();
  return window.location.href = $('#sessionUrl').val() + '/app/'+ $('#comcd').val() +'/830/ACC_ADJUSTVO_THA/index.php?'+code+'=' + value;
}

function searchPartner() {
    keepData();
    if(CSSTYPE.val() == 1) {
        return window.open($('#sessionUrl').val() + '/guide/'+ $('#comcd').val() +'/SEARCHSTAFF/index.php?page=ACC_ADJUSTVO_THA', 'authWindow', 'width=1200,height=600');
    } else if(CSSTYPE.val() == 2) {
        return window.open($('#sessionUrl').val() + '/guide/'+ $('#comcd').val() +'/SEARCHSUPPLIER/index.php?page=ACC_ADJUSTVO_THA', 'authWindow', 'width=1200,height=600');
    } else {
        return window.open($('#sessionUrl').val() + '/guide/'+ $('#comcd').val() +'/SEARCHCUSTOMER/index.php?page=ACC_ADJUSTVO_THA', 'authWindow', 'width=1200,height=600');
    }
}
...
async function action(action) {
    ...
    await axios.post('../ACC_ADJUSTVO_THA/function/index_x.php', data)
    .then(response => {
        // console.log(response.data);
    ...
    .then(response => {
        ...
        $.each(response.data['searchRecur'],function(key, value) {
            ...
            $('#dvwdetail').append( '<tr class="divide-y divide-gray-200" id="rowId'+key+'">'+
                                        ...
                                        '<td class="h-6 w-2/12 pl-1 text-sm border border-slate-700 text-left whitespace-nowrap" id="ACC_NM_TD'+key+'">'+value.ACC_NM+'</td>'+
...
//     await keepData();
//     if(type == 'JV') {
//         var popupWindow = window.open('../ACC_ADJUSTVO_THA/adjust_voucher.php', '_blank', 'width=800, height=800');
//     }
...
async function unsetSession(form) {
    ...
    data.append('systemName', 'ACC_ADJUSTVO_THA');
    await axios.post('../ACC_ADJUSTVO_THA/function/index_x.php', data)
    .then(response => {
        // console.log(response.data)
        clearForm(form);
...
async function clearForm(form) {
    ...
    // refresh
    window.location.href = '../ACC_ADJUSTVO_THA/';
...
function emptyTable() {
    ...
    if (!details.open) { maxrow = 12; } else { maxrow = 8; }
