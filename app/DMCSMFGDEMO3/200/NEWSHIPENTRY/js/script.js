...
SEARCHSALEORDERDETAIL.click(function(e) { e.preventDefault(); winopened = window.open($('#sessionUrl').val() + '/guide/'+ $('#comcd').val() +'/SEARCHSALEORDERDETAIL/index.php?page=NEWSHIPENTRY', 'authWindow', 'width=1200,height=600');});
SEARCHCUSTOMER.click(function(e) { e.preventDefault(); winopened = window.open($('#sessionUrl').val() + '/guide/'+ $('#comcd').val() +'/SEARCHCUSTOMER/index.php?page=NEWSHIPENTRY', 'authWindow', 'width=1200,height=600');});
SEARCHCATALOG.click(function(e) { e.preventDefault(); winopened = window.open($('#sessionUrl').val() + '/guide/'+ $('#comcd').val() +'/SEARCHCATALOG/index.php?page=NEWSHIPENTRY', 'authWindow', 'width=1200,height=600');});
GMAPVIEW.click(function(e) { e.preventDefault(); winopened = window.open($('#sessionUrl').val() + '/guide/'+ $('#comcd').val() +'/GMAPVIEW/index.php?page=NEWSHIPENTRY&GMAPADR=' + $('#GMAPADR').val(), 'authWindow', 'width=1200,height=600');});
...
// form
const form = document.getElementById('newShipEntry');
...
async function getSearch(code, value) {
    $('#loading').show();
    return window.location.href = $('#sessionUrl').val() + '/app/'+ $('#comcd').val() +'/200/NEWSHIPENTRY/index.php?'+code+'=' + value;
}
...
async function getElement(code, value) {
    ...
    await axios.post('../NEWSHIPENTRY/function/index_x.php', data)
    .then(response => {
...
async function unsetSession(form) {
    ...
    data.append('systemName', 'NEWSHIPENTRY');
...
async function clearForm(form) {
    ...
    // refresh
    window.location.href = '../NEWSHIPENTRY/';
    // emptyTable();
