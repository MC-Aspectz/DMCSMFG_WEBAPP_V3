...
function search() {
    ...
    $Param = array( 'DCTYP' => $_POST['DCTYP'],
                    'D1' => !empty($_POST['D1']) ? str_replace('-', '', $_POST['D1']): '',
                    'D2' => !empty($_POST['D2']) ? str_replace('-', '', $_POST['D2']): '',
                  );
    $DataSale = $logicfunc->getDataSale($Param);
    $DataSaleCnt = $logicfunc->getDataSaleCnt($Param);
    // print_r($Param);
    // print_r($DataSaleCnt);
    if(!empty($DataSale)) {
        $data['ITEM'] = $DataSale;
...
function printReport() {
    ...
    $printStatic = $printfunc->printStaticSale($Param);
    $printDynamic = $printfunc->printDynamicSale($Param);
    // print_r($printStatic);
...
function printPDF($printStatic, $printDynamic) {
    ...
    try {
        ...
        for ($x = 1; $x <= $page; $x++) {
            ...
            $report_file = 'SALE_DEBIT_CREDIT_REPORT_'.$x.'_'.date('Ymd_Hi').'.xlsx';
            ...
            // --------------------------------------------------
            // --------------------------------------------------
            $pdf_name = 'SALE_DEBIT_CREDIT_REPORT_'.$x.'_'.date('Ymd_Hi').'.pdf';
