    ...
    <div class="flex flex-col h-screen">
        ...
        <div class="flex flex-1 overflow-hidden">
            ...
            <main class="flex flex-1 overflow-y-auto overflow-x-hidden paragraph px-2">
                ...
                <form class="w-full" method="POST" id="accSaleTaxInfo" name="accSaleTaxInfo" action="" onkeydown="if(event.keyCode == 13) { event.preventDefault(); return false;}">
                    ...
                    <div class="flex mb-1">
                        <div class="flex w-8/12 px-1">
                            ...
                            <select class="text-control text-[12px] shadow-md border mr-1 px-3 h-7 w-2/12 text-left rounded-xl border-gray-300" id="YEARVALUE" name="YEARVALUE">
                            <option value=""></option>
                                <?php foreach ($yearvalue as $yearkey => $yearitem) { ?>
                                    <option value="<?=$yearkey ?>" <?=(isset($data['YEARVALUE']) && $data['YEARVALUE'] == $yearkey) ? 'selected' : '' ?>><?=$yearitem ?></option>
                                <?php } ?>
                            </select>
                            <select class="text-control text-[12px] shadow-md border mr-1 px-3 h-7 w-3/12 text-left rounded-xl border-gray-300 req" id="MONTHVALUE" name="MONTHVALUE" onchange="unRequired();" required>
                            <option value=""></option>
                                <?php foreach ($monthvalue as $monthkey => $monthitem) { ?>
                                    <option value="<?=$monthkey ?>" <?=(isset($data['MONTHVALUE']) && $data['MONTHVALUE'] == $monthkey) ? 'selected' : '' ?>><?=$monthitem ?></option>
                                <?php } ?>
                            </select>
                        </div>
                                ...
                                <?php if(!empty($data['ITEM'])) { $minrow = count($data['ITEM']);
                                    foreach ($data['ITEM'] as $key => $value) { ?>
                                        <tr class="divide-y divide-gray-200 row-id csv" id="rowId<?=$key?>">
                                            ...
                                            <td class="h-6 text-sm border border-slate-700 text-center"><?=isset($value['INVDT']) ? $value['INVDT']: '' ?></td>
                                            <td class="h-6 pl-2 text-sm border border-slate-700 text-left whitespace-nowrap"><?=isset($value['INVNO']) ? $value['INVNO']:'' ?></td>
                                            ...
                                            <td class="hidden"><?=isset($value['DATATYP']) ? $value['DATATYP']: '' ?></td>
                                            <td class="hidden"><?=isset($value['REFINVNO']) ? $value['REFINVNO']: '' ?></td>
                                            ...
                                            <td class="hidden"><?=isset($value['SALEINVDT']) ? $value['SALEINVDT']: '' ?></td>
                        ...
                        <div class="p-1.5 inline-block align-middle">
                            ...
                            <div class="justify-between px-2 border border-gray-200 rounded-xl shadow-sm">
                                ...
                                <summary class="text-color mx-auto py-2 text-lg font-semibold"></summary>                
                                    <div class="flex mb-2 px-2">
                                        <div class="flex w-6/12">
                                            ...
                                            <input class="text-control text-sm shadow-md border z-20 rounded-xl h-7 w-3/12 py-2 px-3 text-gray-700 border-gray-300 read"
                                                   name="INVNO" id="INVNO" value="<?=isset($data['INVNO']) ? $data['INVNO']: ''; ?>" readonly/>
                                            <label class="text-color block text-sm w-3/12 pr-2 pt-1 text-center"><?=checklang('DATE'); ?></label>
                                            <input class="text-control text-sm shadow-md border z-20 rounded-xl h-7 w-3/12 py-2 px-3 text-gray-700 border-gray-300 read"
                                                   type="date" id="SALEINVDT" name="SALEINVDT" value="<?=!empty($data['SALEINVDT']) ? date('Y-m-d', strtotime($data['SALEINVDT'])): ''; ?>" readonly/>
                                        </div>
                                    ...
                                    <div class="flex mb-2 px-2">
                                        <div class="flex w-6/12">
                                            <label class="text-color block text-sm w-3/12 pr-2 pt-1"><?=checklang('INTERNAL_DOCUMENT#'); ?></label>
                                            <input class="text-control text-sm shadow-md border z-20 rounded-xl h-7 w-3/12 py-2 px-3 text-gray-700 border-gray-300 read"
                                                   type="text" name="SVNO" id="SVNO" value="<?=isset($data['SVNO']) ? $data['SVNO']: ''; ?>" readonly/>
                                            <label class="text-color block text-sm w-3/12 pl-4 pt-1"><?=checklang('DOCUMENT_DATE'); ?></label>
                                            <input class="text-control text-sm shadow-md border z-20 rounded-xl h-7 w-3/12 py-2 px-3 text-gray-700 border-gray-300 read"
                                                   type="date" id="SVDT" name="SVDT" value="<?=!empty($data['SVDT']) ? date('Y-m-d', strtotime($data['SVDT'])): ''; ?>" readonly/>
                                        </div>
                    ...
                    <div class="flex my-1 px-2">
                        <div class="flex w-6/12">
                            <button type="button" class="btn text-color border-2 focus:ring-4 focus:outline-none focus:ring-gray-300 font-medium rounded-3xl text-sm px-5 py-1 text-center me-2 mb-2"
                                    id="PRINT" name="PRINT"><?=checklang('PRINT'); ?>
                            </button>
                        </div>
...
<script type="text/javascript">   
    ...
    function unRequired() {
        let month = document.getElementById('MONTHVALUE');
