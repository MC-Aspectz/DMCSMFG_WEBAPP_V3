    ...
    <div class="flex flex-col h-screen">
        ...
        <div class="flex flex-1 overflow-hidden">
            ...
            <main class="flex flex-1 overflow-y-auto overflow-x-hidden paragraph px-2">
                ...
                <form class="w-full" method="POST" id="accWithholdingTaxSlip" name="accWithholdingTaxSlip" action="" onkeydown="if(event.keyCode == 13) { event.preventDefault(); return false;}">
                    <label class="text-color block text-lg font-bold"><?=$_SESSION['APPNAME']; ?></label>
                    <div class="flex mb-1">
                        <div class="flex w-6/12 px-1">
                            <label class="text-color block text-sm w-2/12 pr-2 pt-1"><?=checklang('DATE'); ?></label>
                            <input class="text-control text-sm shadow-md border z-20 rounded-xl h-7 ml-1 w-4/12 py-2 px-3 text-gray-700 border-gray-300 req"
                                   type="date" name="D1" id="D1" value="<?=!empty($data['D1']) ? date('Y-m-d', strtotime($data['D1'])): ''; ?>" onchange="unRequired();" required/>
                            <label class="text-color block text-sm pr-2 ml-3 pt-1">→</label>
                            <input class="text-control text-sm shadow-md border z-20 rounded-xl h-7 ml-1 w-4/12 py-2 px-3 text-gray-700 border-gray-300 req"
                                   type="date" name="D2" id="D2" value="<?=!empty($data['D2']) ? date('Y-m-d', strtotime($data['D2'])): ''; ?>" onchange="unRequired();" required/>
                        </div>
                    ...
                    <div id="table-area" class="overflow-scroll px-2 block h-[336px]">
                        <table id="table" class="w-full border-collapse border border-slate-500 divide-gray-200 wts_tb" id="table" rules="cols" cellpadding="3" cellspacing="1" >
                            <thead class="sticky top-0 bg-gray-50">
                                <tr class="border border-gray-600">
                                    ...
                                    <th class="px-3 text-center border border-slate-700">
                                        <span class="text-color text-sm font-semibold tracking-wide whitespace-nowrap"><?=checklang('PAY_AMOUNT'); ?></th></span>
                                    <th class="px-3 text-center border border-slate-700">
                                    ...
                                    <th class="px-3 text-center border border-slate-700">
                                        <span class="text-color text-sm font-semibold tracking-wide whitespace-nowrap"><?=checklang('SUBMIT_TO_MONTH'); ?>
                                    </th></span>
                                </tr>
                            </thead>
                                ...
                                <?php if(!empty($data['ITEM'])) {
                                    ...
                                    foreach ($data['ITEM'] as $key => $value) { ?>
                                        <tr class="border border-gray-600 row-id" id="rowId<?=$key?>">
                                            ...
                                            <td class="h-6 w-1/12 pl-1 text-sm border border-slate-700 whitespace-nowrap text-left"><?php foreach ($monthvalue as $month => $monthItem) { if($value['PAYMENTADD11'] == $month) { echo $monthItem; } }?></td>
                                ...
                                <summary class="text-color mx-auto py-2 text-lg font-semibold"></summary>  
                                    ...
                                    <div class="flex mb-2">
                                        <div class="flex w-6/12">
                                            ...
                                            <input type="hidden" id="PAYMENTNOLN" name="PAYMENTNOLN">
                                    ...
                                    <div class="flex mb-2">
                                        <div class="flex w-6/12">
                                            ...
                                            <label class="text-color block text-sm w-3/12 pl-4 pt-1"><?=checklang('PAYMENT_TYPE'); ?></label>
                                            <input class="text-control text-sm shadow-md border z-20 rounded-xl h-7 w-3/12 py-2 px-3 text-gray-700 border-gray-300 read" 
                                                   type="text" id="PMNOTE02" name="PMNOTE02" value="<?=isset($data['PMNOTE02']) ? $data['PMNOTE02']: ''; ?>" readonly/>
                                        </div>

                                        <div class="flex w-6/12">
                                            <label class="text-color block text-[12px] w-3/12 pr-2 pt-1"><?=checklang('INTERNAL_DOCUMENT#'); ?></label>
                                            <input class="text-control text-sm shadow-md border z-20 rounded-xl h-7 w-4/12 py-2 px-3 text-gray-700 border-gray-300 read"
                                                   type="text" name="PAYMENTNO" id="PAYMENTNO" value="<?=isset($data['PAYMENTNO']) ? $data['PAYMENTNO']: ''; ?>" readonly/>
                                            <label class="text-color block text-sm w-2/12 pl-4 pt-1"><?=checklang('DATE'); ?></label>
                                            <input class="text-control text-sm shadow-md border z-20 rounded-xl h-7 w-3/12 py-2 px-3 text-gray-700 border-gray-300 read"
                                                   type="date" id="PAYMENTDT" name="PAYMENTDT" value="<?=!empty($data['PAYMENTDT']) ? date('Y-m-d', strtotime($data['PAYMENTDT'])): ''; ?>" readonly/>
                                        </div>
                                    </div>

                                    <div class="flex mb-2">
                                        <div class="flex w-6/12">
                                            <label class="text-color block text-sm w-2/12 pr-2 pt-1"><?=checklang('TAX_AMT'); ?></label>
                                            <input class="text-control text-sm shadow-md border rounded-xl h-7 w-3/12 py-2 px-3 text-gray-700 border-gray-300 read"
                                                   type="text" name="PAYMENTAMT" id="PAYMENTAMT" value="<?=isset($data['PAYMENTAMT']) ? $data['PAYMENTAMT']: ''; ?>" readonly/>
                                            <label class="text-color block text-sm w-3/12 pl-4 pt-1"><?=checklang('WHTRATE'); ?></label>
                                            <input class="text-control text-sm shadow-md border z-20 rounded-xl h-7 w-3/12 py-2 px-3 text-gray-700 border-gray-300 read"
                                                   type="text" id="PMNOTE05" name="PMNOTE05" value="<?=isset($data['PMNOTE05']) ? $data['PMNOTE05']: ''; ?>" readonly/>
                                        </div>
                                        
                                        <div class="flex w-6/12">
                                            <label class="text-color block text-sm w-3/12 pr-2 pt-1"><?=checklang('TAX_CONDITION'); ?></label>
                                            <select class="text-control text-[12px] shadow-md border px-3 h-7 w-4/12 text-left rounded-xl border-gray-300 read" id="PAYMENTADD14" name="PAYMENTADD14">
                                            <option value=""></option>
                                            ...
                                            <label class="text-color block text-sm w-2/12 pl-4 pt-1"><?=checklang('TAX_GROUP'); ?></label>
                                            <input class="text-control text-sm shadow-md border z-20 rounded-xl h-7 w-3/12 py-2 px-3 text-gray-700 border-gray-300 read"
                                                   type="text" name="PAYMENTTYP" id="PAYMENTTYP" value="<?=isset($data['PAYMENTTYP']) ? $data['PAYMENTTYP']: ''; ?>" readonly/>
                                        </div>
                                    </div>
                                    ...
                                    <div class="flex mb-2">
                                        <div class="flex w-6/12">
                                            ...
                                            <input class="text-control text-sm shadow-md border z-20 rounded-xl h-7 w-3/12 py-2 px-3 text-gray-700 border-gray-300 text-right"
                                                   type="text" name="PAYMENTADD09" id="PAYMENTADD09" value="<?=isset($data['PAYMENTADD09']) ? $data['PAYMENTADD09']: ''; ?>"
                                                    onchange="this.value = num2digit(this.value);" oninput="this.value = stringReplacez(this.value);"/>
                                            <label class="text-color block text-sm w-3/12 pl-4 pt-1"><?=checklang('PROVIDED_FUND'); ?></label>
                                            <input class="text-control text-sm shadow-md border z-20 rounded-xl h-7 w-3/12 py-2 px-3 text-gray-700 border-gray-300 text-right"
                                                   type="text" id="PAYMENTADD10" name="PAYMENTADD10" value="<?=isset($data['PAYMENTADD10']) ? $data['PAYMENTADD10']: ''; ?>"
                                                    onchange="this.value = num2digit(this.value);" oninput="this.value = stringReplacez(this.value);"/>           
                                        </div>

                                        <div class="flex w-6/12">
                                            <label class="text-color block text-sm w-3/12 pr-2 pt-1"><?=checklang('SUBMIT_TO_MONTH'); ?></label>
                                            <select class="text-control text-[12px] shadow-md border px-3 h-7 w-4/12 text-left rounded-xl border-gray-300" id="PAYMENTADD11" name="PAYMENTADD11">
                                            <option value=""></option>
                                            ...
                                            <label class="text-color block text-sm w-2/12 pl-4 pt-1"><?=checklang('TAX_TYPE'); ?></label>
                                            <select class="text-control text-[12px] shadow-md border px-3 h-7 w-3/12 text-left rounded-xl border-gray-300 read" id="PAYMENTADD13" name="PAYMENTADD13">
                                            <option value=""></option>
                    ...
                    <div class="flex mt-1 px-2">
                        <div class="flex w-6/12">
                            ...
                            <button type="button" class="btn text-color border-2 focus:ring-4 focus:outline-none focus:ring-gray-300 font-medium rounded-3xl text-sm px-5 py-1 text-center me-2 mb-2"
                                    id="PND3" name="PND3" ><?=checklang('PND3')?></button>
                            <button type="button" class="btn text-color border-2 focus:ring-4 focus:outline-none focus:ring-gray-300 font-medium rounded-3xl text-sm px-5 py-1 text-center me-2 mb-2"
...
<script type="text/javascript">   
    ...
    function actionDialog(type) {
        if(type == 1) {
           ...
           return questionDialog(3, '<?=lang('question2')?>', '<?=lang('yes'); ?>', '<?=lang('no'); ?>'); 
            ...
            // PND53
            return questionDialog(4, '<?=lang('question2')?>', '<?=lang('yes'); ?>', '<?=lang('no'); ?>'); 
        } else {
    ...
    function unRequired() {
        document.getElementById('D1').classList[document.getElementById('D1').value != '' ? 'remove' : 'add']('req');
        document.getElementById('D2').classList[document.getElementById('D2').value != '' ? 'remove' : 'add']('req');
    }
</script>
</html>