// form
const form = document.getElementById('accWithholdingTaxSlip');
...
async function update() {
    ...
    await axios.post('../ACC_INPUTWITHHOLDINGTAXSLIP/function/index_x.php', data)
    .then(response => {
...
async function printed(type) {
    ...
    if(type == 'Export') {
        ...
        var popupWindow = window.open('../ACC_INPUTWITHHOLDINGTAXSLIP/print_wht.php', '_blank', "width=800, height=800");
    } else if(type == 'PND53') {
        var popupWindow = window.open('../ACC_INPUTWITHHOLDINGTAXSLIP/print_pnd53.php', '_blank', "width=800, height=800");
    }
...
function selectRow() {
    $('table#table tr').click(function () {
        ...
        if(id != '') {
            ...
            $('#PAYMENTNOLN').val(item.eq(16).text());
...
async function unsetSession(form) {
    ...
    data.append('systemName', 'ACC_INPUTWITHHOLDINGTAXSLIP');
...
async function clearForm(form) {
    ...
    emptyTable();
    $('#record').html(0);
    // window.location.href = '../ACC_INPUTWITHHOLDINGTAXSLIP/';
    unRequired();
    return false;
}
