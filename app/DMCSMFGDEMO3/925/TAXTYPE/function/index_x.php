...
if(!empty($_GET)) {
    ...
    if(isset($_GET['refresh'])) {
        // $data = getSessionData();
        $TAXTYPESEARCH = isset($data['TAXTYPESEARCH']) ? $data['TAXTYPESEARCH']: '';

        $query = $javaFunc->search($TAXTYPESEARCH);
        $data['TAX'] = $query;
        setSessionArray($data); 
    }
// onchange
        ...
        $excute = $javaFunc->get($data['TAXTYPECD']);
        if(!empty($excute)){
            $data =$excute;
        }
    ...
    else if(isset($_GET['VATRATE'])) {

        $data['VATRATE'] = isset($_GET['VATRATE']) ? $_GET['VATRATE']: '';
        $excute = $javaFunc->ratechk($_GET['VATRATE']);
        $data =$excute;
...
// dropdown
// $type = getDropdownData("TAX02");
// if(empty($type)) {
//     $type = $syslogic->getPullDownData('TAX02', $_SESSION['LANG']);
//     setDropdownData("TAX02", $type);
// }

// $cate = getDropdownData("TAXTTL");
// if(empty($cate)) {
//     $cate = $syslogic->getPullDownData('TAXTTL', $_SESSION['LANG']);
//     setDropdownData("TAXTTL", $cate);
// }


// TAXTYPECD,TAXTYPENAME,TAXKBN,VATRATE,TAXTTL
function insert() {
    $javaInsrt = new TaxType;
    $Param = array( "TAXTYPECD" => $_POST['TAXTYPECD'],
                    "TAXTYPENAME" => $_POST['TAXTYPENAME'],
                    "TAXKBN" => $_POST['TAXKBN'],
                    "VATRATE" => $_POST['VATRATE'],
                    "TAXTTL" => $_POST['TAXTTL']);
                    // print_r($Param);
    $insert = $javaInsrt->ins($Param);
