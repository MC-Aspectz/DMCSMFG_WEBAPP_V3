...
<div class="flex flex-col h-screen">
    ...
    <div class="flex flex-1 overflow-hidden">
        ...
        <!--   start::Main Content  -->
        <main class="flex flex-1 paragraph">
            <input type="hidden" id="appcode" name="appcode" value="<?=$appcode?>">
            <input type="hidden" id="comcd" name="comcd" value="<?=$_SESSION['COMCD']?>">
            <input type="hidden" id="sessionUrl" name="sessionUrl" value="<?=$_SESSION['APPURL']?>">
            <form class="flex w-full" method="POST" id="productionOrderEntry" name="productionOrderEntry" action="" onkeydown="if(event.keyCode == 13) { event.preventDefault(); return false;}">
                <!-- Side Search -->
                <aside class="w-4/12 px-2 overflow-y-auto border-r side-<?=$appcode?>">
                    <div class="flex my-2">
                        <label class="text-color block text-sm font-semibold w-3/12 pr-2 pt-1"><?=checklang('INPUT_DATE')?></label>
                        <input type="date" class="text-control shadow-md border z-20 rounded-xl h-7 w-4/12 py-2 px-3 text-gray-700 border-gray-300 focus:ring-blue-500 focus:border-blue-500 text-center"
                                id="P1" name="P1" value="<?=!empty($data['P1']) ? date('Y-m-d', strtotime($data['P1'])) : ''; ?>"/>&ensp;→&ensp;
                        <input type="date" class="text-control shadow-md border z-20 rounded-xl h-7 w-4/12 py-2 px-3 text-gray-700 border-gray-300 focus:ring-blue-500 focus:border-blue-500 text-center"
                                id="P2" name="P2" value="<?=!empty($data['P1']) ? date('Y-m-d', strtotime($data['P1'])) : ''; ?>" onchange="searchProOrder();"/>
                    </div>
                   <table id="table-side" class="table_side w-full">
                        <tbody id="sidedetail"><?php
                            if(!empty($data['SEARCHITEM'])) {
                                foreach($data['SEARCHITEM'] as $key => $value) { ?>
                                <!-- PROITEMNAME -->
                                <tr class="py-2 px-2 cursor-pointer">
                                    <td class="hidden"><?=$key ?></td>
                                    <td class="w-5/12 pl-2 text-md whitespace-nowrap"><?=isset($value['PROORDERNO']) ? $value['PROORDERNO']: '' ?></td>
                                    <td class="w-7/12 pl-2 text-md whitespace-nowrap"><?=isset($value['PROITEMCD']) ? $value['PROITEMCD']: '' ?></td>
                                </tr>
                                <?php }
                            }?>
                        </tbody>
                 ...
                 <!-- Content Page -->
                <article class="w-8/12 overflow-y-auto">
                    <button type="button" class="label-app text-color block text-lg font-bold mx-2" onclick="javascript:toggleSideForm('<?=$appcode?>');"><?=$_SESSION['APPNAME']; ?></button>
                    <div class="flex mb-1">
                        <div class="flex w-6/12 px-2">
                            <label class="text-color block text-sm w-4/12 pr-2 pt-1"><?=checklang('PRODUCT_ORDER')?></label>
                            <div class="relative w-7/12 mr-1">
                        ...
                        <div class="flex w-6/12 px-2">
                            ...
                            <label class="text-color block text-sm w-3/12 pr-2 pt-1"><?=checklang('INPUT_DATE')?></label>
                    ...
                    <div class="flex mb-1">
                        <div class="flex w-6/12 px-2">
                            <label class="text-color block text-sm w-4/12 pr-2 pt-1"><?=checklang('BRANCH_TYPE')?></label>
                            <select class="text-control text-[12px] shadow-md border px-2 h-7 w-7/12 text-left rounded-xl border-gray-300 req" id="PROFACTYP" name="PROFACTYP" onchange="unRequired();" required>
                                <option value=""></option>
...
<script type="text/javascript">
    ...
    $(document).on('click', '.table_side tbody tr', async function(event) {
        $('#table-side tbody tr').not(this).removeClass('item-selected'); 
        let item = $(this).closest('tr').children('td');
        let rec = item.eq(0).text();
        // console.log(rec);
        let table = document.getElementById('table-side');
        if(rec != '') { 
            table.rows[rec].classList.toggle('item-selected');
            let PROORDRNO = item.eq(1).text();
            await getElementNO('PROORDERNO', PROORDRNO);
        }
    });
    ...
    function HandlePopupResult(code, result) {
        ...
        if(code == 'PROORDERNO') {
            return getElementNO(code, result);
        } else {
