...
<body>
    ...
    <form class="form" method="POST" id="storage_master" onkeydown="if(event.keyCode == 13) { event.preventDefault(); return false;}">
    <div class="col-md-12">
        
            <div class="flex">
            <div class="flex col-first">            
            <label class="label-width27"><?=$data['TXTLANG']['LO_CODE'];?></label>
            <input class="form-control width26" type="text" id="STORAGECD_S" name="STORAGECD_S" value="<?=$STORAGECD_S?>" />
             
                <div class="fix-icon">
                <a href="#" id="searchstorage"><img style="img-height20" src="../../../../img/search.png"></a>
                </div>
              </div>
                    <div class="flex col-first">
              ...
              <table class="table-head table-striped" id="search_table" rules="cols" cellpadding="3" cellspacing="1">
                  <thead>
                      <tr class="table-secondary">
                             <th style="text-align: center; "><?=$data['TXTLANG']['LO_CODE']; ?></th>
                            <th style="text-align: center; "><?=$data['TXTLANG']['LO_NAME']?></th>
                            <th style="text-align: center; "><?=$data['TXTLANG']['DIVISIONCODE']?></th>
                            <th style="text-align: center; "><?=$data['TXTLANG']['DIVISIONNAME']?></th>                        
                      </tr>
                  </thead>
                  <tbody>
                  <?php if(!empty($data['STORE']))  {
                        foreach ($data['STORE'] as $key => $value) {
                            if(is_array($value)) {
                              $minrow = count($data['STORE']) + 1;
                             ?>
                              <tr class="tr_border table-secondary">
                                  <td class="td-class"><?=isset($value['STORAGECD']) ? $value['STORAGECD']: '' ?></td>
                                  <td class="td-class"><?=isset($value['STORAGENAME']) ? $value['STORAGENAME']: '' ?></td>
                                ...
                                $minrow = 1; ?>
                                <tr class="tr_border table-secondary">
                                      <td class="td-class"><?=$data['STORE']['STORAGECD'] ?></td>
                                      <td class="td-class"><?=$data['STORE']['STORAGENAME'] ?></td>
                                      <td class="td-class"><?=$data['STORE']['DIVISIONCD'] ?></td>
                                      <td class="td-class"><?=$data['STORE']['DIVISIONNAME'] ?></td>
        ...
        <div class="d-flex p-2">
            <div class="flex">
                <div class="flex .col55">
                    <label class="label-width27"><?=$data['TXTLANG']['LO_CODE']; ?></label>&nbsp;&nbsp;&nbsp;&nbsp;
                    <input class="form-control width20 req" type="text" id="STORAGECD" name="STORAGECD"
                     value="<?=isset($data['STORAGECD']) ? $data['STORAGECD']: ''?>" required  />
                </div>
        ...
        <div class="d-flex p-2">
            <div class="flex">
                <div class="flex .col55">
                    <label class="label-width27"><?=$data['TXTLANG']['LO_NAME']?></label>&nbsp;&nbsp;&nbsp;&nbsp;
                     <input class="form-control width43 req" type="text" id="STORAGENAME" name="STORAGENAME" value="<?=isset($data['STORAGENAME']) ? $data['STORAGENAME']: ''?>" required />
                </div>
        ...
        <div class="d-flex p-2">
           <div class="flex">
           <div class="flex .col55">
                    <label class="label-width27"><?=$data['TXTLANG']['DIVISIONCODE']?></label>&emsp;
                    <input class="form-control width20 " type="text" id="DIVISIONCD" name="DIVISIONCD" value="<?=isset($data['DIVISIONCD']) ? $data['DIVISIONCD']: ''?>" required/>                
                <div class="fix-icon">
